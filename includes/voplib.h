...
/* APIs that take an hCodec returned by DSCodecCreate() or a codec type. Notes:

   -DSGetCodecInfo() and DSGetPayloadInfo() accept either codec types or hCodec handles depending on uFlags
   -a codec instance is an hCodec handle from a previous call to DSCodecCreate(), a codec type is one of the DS_VOICE_CODEC_xxx or DS_VIDEO_CODEC_xxx definitions in shared_include/codec.h
...
/* DSGetPayloadHeaderToC() returns a nominal AMR or EVS payload header ToC based on payload size. For EVS, this API should be called *only* with compact header mode and non-collision payload sizes. The ToC is normally one byte, so it's returned in the low byte of the return value, but could be larger for other codecs if supported. See the AMR and EVS specs for ToC bit fields definitions */
