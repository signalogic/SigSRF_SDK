...
Codecs are an essential component in streaming media use cases, necessary to decode RTP media in telecom, analytics, robotics, and many other applications. In addition, codecs are often necessary to encode media, for example transcoding in SBC, language translation, home and mobile assistants, and other applications. This page documents SigSRF codecs and RTP media formats as both stand-alone libraries for user applications and as linked with SigSRF reference applications, including mediaMin and mediaTest. The voplib API, documented here, makes it straightforward to incorporate EVS, AMR, AMR-WB, G711, H.264, H.265 (HEVC) for production, test and measurement, intelligence, or other use cases on a variety of platforms, including bare metal, VM, or cloud. Documentation is organized as:
