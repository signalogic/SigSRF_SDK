...
x86 software is sometimes referred to as a "software only" solution, but that's an Intel marketing term. In reality there is no software without hardware.  With the recent surge in deep learning / neural net chips attempting to emulate the human brain, hardware limitations have never been more apparent, certainly a major reason current AI technology dramatically fails to equal biological intelligence.  In general, all HPC applications face these contraints -- for 30 years people have failed to solve this with generic x86 processors, and success isn't likely to happen any time soon.
