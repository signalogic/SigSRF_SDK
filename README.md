...
where N is the target number of concurrent streams and memSize is RAM in GB. For example, a single CPU socket server processing 2000 concurrent streams needs 128 GB of RAM and a 64 core CPU; a server with two (2) CPUs needs 64 GB per CPU and 32 cores per CPU. For applications with server memory or core constraints, custom builds are possible to achieve tradeoffs between capacity and functionality.
