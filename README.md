...
SigSRF library modules support both a packet timestamp timing mode, and a clockless (data driven) mode in which packet timestamps are ignored.  Timestamp timing is used for telecom and media delivery applications where hard real-time performance and low latency are required.  Clockless timing is used by data analytics, lawful interception, web IT, and other applications that are "one step removed" from original end-point / source timing.
...
Some notes about the above data flow diagram:
   ...
   5) <b>Multithreaded</b>.  A copy of the above diagram runs per thread, where each thread typically consumes one CPU core in high performance applications.
