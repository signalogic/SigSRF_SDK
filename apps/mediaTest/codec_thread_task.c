...
extern char* encoder_input_files[];
extern char* decoder_input_files[];

extern int encoded_frame_cnt[];
extern int decoded_frame_cnt[];
...
/* Skip EVS header in input file - only for file i/o operations with decoder */
   ...
   while (pm_run && ((ret_val = _fread(in_buf, sizeof(char), in_frame_size, in_fp)) == (int)in_frame_size)) {
      ...
      out_frame_size = DSCodecDecode(&hCodec, 0, in_buf, out_buf, in_frame_size, 1, NULL, NULL);
