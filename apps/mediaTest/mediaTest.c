...
/* code start - display banner messages, process command line */
   ...
   sprintf(lib_info, "  SigSRF libraries in use: DirectCore v%s", HWLIB_VERSION);
   #ifndef _NO_PKTLIB_  /* JHB Aug 2022 */
   sprintf(&lib_info[strlen(lib_info)], ", pktlib v%s", PKTLIB_VERSION);
   #endif
   #ifndef _NO_MEDIAMIN_
   sprintf(&lib_info[strlen(lib_info)], ", streamlib v%s", STREAMLIB_VERSION);
   #endif
   sprintf(&lib_info[strlen(lib_info)], ", voplib v%s, alglib v%s, diaglib v%s, cimlib v%s", VOPLIB_VERSION, ALGLIB_VERSION, DIAGLIB_VERSION, CIMLIB_VERSION);
   #ifdef _ALSA_INSTALLED_
   sprintf(&lib_info[strlen(lib_info)], ", aviolib v%s", AVIOLIB_VERSION);
   #endif
   ...
   if (!cmdLineInterface(argc, argv, CLI_MEDIA_APPS | CLI_MEDIA_APPS_MEDIATEST, version_info, banner_info)) exit(EXIT_FAILURE);
   {
      ...
      Log_RT(4, "mediaTest INFO: event log setup complete, log file %s, %s %s ", sig_lib_event_log_filename, strlen(szAppFullCmdLine) ? "cmd line" : "-dN cmd line options", tmpstr);
   }
