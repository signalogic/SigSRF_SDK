...
# generate .wav and .pcap files
#
# usage example:
# cd /signalogic_software_installpath/sigsrf_sdk_demo/apps/mediaTest
# source ./evs_interop_test.sh

#mediaTest outputs: replace with output path as needed. Default is mediaTest/test_files subfolder (installed with the SDK)
MT_OUTPUTS="test_files"

# mediaMin outputs: replace with system-specific locations (for example RAM disk), or assign to empty string to store output files on mediaMin subfolder
...
cd ..
#
# 13200 bps tests, including RF enable mode
#
