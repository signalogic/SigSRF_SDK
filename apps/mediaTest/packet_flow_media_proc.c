...
bool fNewLine = false, fNextLine = false;
   ...
   if ((level & PRN_LEVEL_MASK) > lib_dbg_cfg.uPrintfLevel) return;  /* default level should be set to 5. See above at start of packet_flow_media_proc() and also mediaMin.cpp. 'o' interactive key in mediaMin sets uPrintfLevel to "none", effectively turning off p/m thread screen output */
