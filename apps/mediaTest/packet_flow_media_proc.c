/*
 ...
 Copyright (C) Signalogic Inc. 2017-2025
 ...
 Revision History
  ...
  Modified Nov 2018 JHB, add CheckForDormantSSRC() to detect and flush channels with dormant SSRCs that are duplicated by another channel, for example due to call-waiting or on-hold situations
  ...
  Modified Mar 2021 JHB, in DSLogRunTimeStats() incorporate use of DS_LOG_RUNTIME_STATS_xx flags
  ...
  Modified Apr 2024 JHB, several updates and mods to verify functionality in mediaTest mode (look for PUSHPACKETS_REFERENCE below). This fixes the "EVS Player" and "AMR Player" command lines on the Github page. Also remove a "continue" in the packet + payload processing loop that was preventing correct packet logging (look for pcap_index < 0)
  ...
  Modified Sep 2024 JHB, fix bug in calculating transrating between decode and encode stream RTP timestamps before calling DSFormatPacket(). See comments near rtp_clockrate_decoder and rtp_clockrate_encoder
  Modified Nov 2024 JHB, add DS_PKT_PYLD_CONTENT_MEDIA_REUSE to checks for DS_PKT_PYLD_CONTENT_MEDIA. Oversight, should have been done in Apr 2024 when media reuse was added
  Modified Nov 2024 JHB, re-add session_uFlags[] to support optimized session flag reference after it was somehow removed. This fixes a bug where some code was incorrectly using term_uFlags[][] (termination flags) in place of session flags. This was a major bug but fortunately only affected static sessions. Also modify ManageSessions() to regularly update session_uFlags[] as with term_uFlags[][]
  Modified Nov 2024 JHB, use IPvN_ADDR_XXX definitions in pktlib.h
  Modified Dec 2024 JHB, in DSLogRunTimeStats() run-time session stats display, moved "fDeletePending" inside the session do-loop to fix bug with stats for all sessions not being fully displayed in some cases
  Modified Dec 2024 JHB, add DS_BUFFER_PKT_EXTENDED_SESSION_SEARCH flag in DSBufferPackets() uFlags. See pktlib.h (see also DS_PKT_INFO_EXTENDED_SESSION_SEARCH definition)
  ...
  Modified Apr 2025 JHB, in sig_printf() fixes and simplification to line cursor position update, mid-line check, and isLinePreserve
  Modified Apr 2025 JHB, remove analytics mode exception in p/m worker thread console status update printout. This should be a performance increase for various operating modes that use analytics mode (OpenLI, average push rate, timestamp matching, etc); it was found and debugged using mediaMin -r0 cmd line operation, where PushPackets() in mediaMin.cpp pushes packets as fast as possible, with no timing contraints (called "AFAP" mode in mediaMin.cpp, look for isAFAPMode()). See comments near ANALYTICS_MODE_STATUS_UPDATE_EXCEPTION
  Modified May 2025 JHB, in DSBufferPackets() propagate results of DSGetPayloadInfo(), for example voice.fDTMF, and in p/m worker thread loop remove all DSGetPacketInfo() calls with DS_PKT_INFO_RTP_PYLD_CONTENT. Notes:
                         -DSPayloadInfo() is the ultimate arbiter, especially if codec type is necessary to make the decision
...
#include <stdio.h>  /* includes string.h */
#include <stdarg.h>
#include <stdint.h>
...
#include "directcore.h"  /* DirectCore lib header file (all libs are .so format) */
...
#include "minmax.h"     /* min and max macros defined only if gcc is compiling */
...
/* maximum number of sessions handled by pktlib concurrently is defined by MAX_SESSIONS in shared_include/transcoding.h and maximum number of streams by MAX_STREAMS in shared_include/streamlib.h. This includes reference applications and multithread / high capacity testing */
...
/* Logging notes:
  ...
  2) If an application does not define/enable event logging, ptklib uses the LOG_OUTPUT define (below) to enable and manage Log_RT() output (default
     definition is LOG_CONSOLE_FILE). In this case, applications are responsible for opening and closing log files, and if so, then passing a log file
...
//#define LOG_OUTPUT  LOG_CONSOLE_ONLY  /* console output only */
//#define LOG_OUTPUT  LOG_FILE_ONLY     /* event log file output only */
#define LOG_OUTPUT  LOG_CONSOLE_FILE  /* console + event log file output */
...
extern MEDIAPARAMS MediaParams[MAX_STREAMS];
#endif
...
uint32_t uStreamSyncCompensations[NCORECHAN] = { 0 };
#endif
...
static FILE*          fp_in[MAX_STREAMS] = { NULL }, *fp_out[MAX_STREAMS] = { NULL };
static unsigned int   link_layer_length[MAX_STREAMS];
...
#define isMasterThread(thread_index) (thread_index == 0)
...
void RecordPacketTimeStats(int chnum, uint8_t* pkt_ptr, int packet_len, unsigned int pkt_info, uint32_t pkt_count, unsigned int uFlags);
...
#if 0  /* now declared as inline in pktlib.h, JHB Jul 2025 */
...
extern inline __attribute__((unused)) uint32_t get_session_thread_index(HSESSION hSession);  /* in pktlib.c, used by set_session_last_push_time() to support unified timebase. Currently set_session_last_push() is called by DSPushPackets() in pktlib, JHB Jun 2023 */
#endif
#endif
...
static int nSSRCDuplicationDetection[NCORECHAN] = { 0 };
...
static uint8_t session_run_time_stats[NCORECHAN] = { 0 };  /* keep track of session stats logging, avoid repeating each time DSLogRunTimeStats() is called */
...
static uint32_t uSSRCDuplicationCount[NCORECHAN] = { 0 };  /* session SSRC duplication and reuse stats, JHB Jul 2025 */
static uint32_t uSSRCReuseCount[NCORECHAN] = { 0 };
...
*/
   ...
   int ret_val_wav;
   MEDIAINFO MediaInfo[MAX_STREAMS] = {{{ 0 }}};
   ...
   int data_length, packet_length, packet_len[MAX_PACKETS_PER_PULL];
   unsigned int pkt_info[MAX_PACKETS_PER_PULL];
   uint32_t rtp_timestamp[MAX_PACKETS_PER_PULL], rtp_ssrc[MAX_PACKETS_PER_PULL];
   uint8_t rtp_pyld_type[MAX_PACKETS_PER_PULL];
   ...
   int pkt_pulled_cnt = 0, pkt_xcode_cnt = 0, pkt_passthru_cnt = 0, pkt_group_cnt = 0, pkt_bitstream_cnt = 0;
   ...
   #endif
   unsigned int uFlags_buffer_pkts, uFlags_info, uFlags_get, uFlags_format;

   int stream_indexes[MAX_STREAMS] = { 0 };

   int chan_nums[MAX_TERMS+NCORECHAN] = { 0 };  /* allow for dynamic channels */

   int32_t pkts_read[MAX_STREAMS] = { 0 };

   unsigned int sample_rate[MAX_STREAMS] __attribute__ ((unused)) = { 0 };
   FORMAT_PKT formatPkt = { 0 };

#ifndef PKT_COUNTERS_GLOBAL  /* moved to global var, indexed by thread, to allow use as param in DSPktStatsWriteLogFile(), JHB Dec 2019 */
...
/* stream audio data merging declarations */
   ...
   #ifdef ANALYTICS_MODE_STATUS_UPDATE_EXCEPTION
   bool fAnalyticsMode = false;
   #endif
   bool fAllSessionsDataAvailable = true;  /* set by ManageSessions() inside the p/m thread loop, but we declare it here because it's used in earlier in the loop (console status/progress counter update conditions), JHB Apr 2025 */
      ...
      while (!fFound) {  /* find our thread index. note we need to wait until DSConfigMediaService() has initialized mediaThreads[] */
         ...
         for (i=0; i<MAX_PKTMEDIA_THREADS; i++) {
            ...
            if (pthread_equal(packet_media_thread_info[i].threadid, pthread_self())) {
               ...
               sem_wait(&pktlib_sem);  /* protect any mod to num_pktmedia_threads. ManageSessions() runs immediately upon creation of first p/m thread, JHB Jan 2020 */

               num_pktmedia_threads++;
               num_pktmedia_threads_local =  num_pktmedia_threads;

               DSCreateFilelibThread();  /* establish a thread index entry in filelib, this allows us to avoid semaphore usage when calling DSLoadDataFile() and DSSaveDataFile() (hwlib) which in turn call filelib APIs DSOpenFile, DSCloseFile, DSWriteWvfrmHeader(), etc, JHB Jan 2020 */
...
/* when running as a thread discover any pre-existing sessions created by the user app before DSConfigMediaService() was called (i.e. API in pktlib that starts the thread) */
   ...
   if (fMediaThread) {
      ...
      if (!isMasterThread(thread_index)) goto time_init;  /* only master thread does pre-existing session init */
...
/* open output pcap and/or wav files, stop on first failure (but still allow program to run) */
   ...
   #pragma GCC diagnostic pop
      ...
      if (strstr(strupr(strcpy(tmpstr, MediaParams[nOutFiles].Media.outputFilename)), ".PCAP") && packet_media_thread_info[thread_index].packet_mode) {
         ...
         MediaInfo[nOutFiles].Fs = WB_CODEC_FS;  /* set default sampling rate (in Hz), update wav header after decoding, when sampling rate becomes known */
   ...
   if (fNetIOAllowed && isMasterThread(thread_index))
   #endif
   {
...
set_session_flags:
   ...
   for (i = threadid; i < numStreams; i += nThreads_gbl) {
      ...
      if (InitStream(hSessions_t, i, thread_index,
      #ifdef ANALYTICS_MODE_STATUS_UPDATE_EXCEPTION
      &fAnalyticsMode
      #else
      NULL
...
/* determine number of static sessions and streams associated with static sessions, JHB Jan 2023 */
   ...
   for (i = threadid; i < nSessionsCreated; i += nThreads_gbl) {
      ...
      for (j=0; j<MAX_TERMS; j++) {
         ...
         #else
         if (!(term_uFlags[hSessions_t[i]][j] & TERM_DYNAMIC_SESSION)) num_static_streams++;
         #endif
         #endif
      }
   }

   sprintf(tmpstr, "Static session initialization: %d session%s and %d stream termN endpoint%s \n", nSessionsInit, nSessionsInit != 1 ? "s" : "", num_static_streams, num_static_streams != 1 ? "s" : "");
...
/* continuous packet/media thread loop */
   ...
   do {
      ...
      if (pm_run == 99) continue;  /* reserved for system stall stress testing (p/m threads are stalled / preempted for whatever reason), JHB Apr 2020 */

   /* progress counter and status console update */

      if (
          #ifdef ANALYTICS_MODE_STATUS_UPDATE_EXCEPTION  /* this is only a label for documentation purposes, not intended to be defined. I'm not sure why analytics mode is any exception, maybe this is left over from early days. It definitely impacts mediaMin -r0 cmd line operation, where PushPackets() in mediaMin.cpp pushes packets as fast as possible, without any timing constraints. In that case, p/m threads on every iteration are constantly pulling one or more packets and printing to console, which is a bad idea. Testing will show if this has any negative effects I'm not thinking about yet, JHB Apr 2025 */
          fAnalyticsMode ||
          #else
          !fAllSessionsDataAvailable ||
          #endif
          (cur_time - prev_display_time > 20000*timeScale)) {  /* print counters and check keyboard input every 20 msec */
         ...
         {
            bool fAnyData = false;
            char leading_space_str[100] = "";

            if (fMediaThread) {

               if (uApp_progress_line_cursor_pos > 0) {  /* p/m threads print one space after apps on progress line if they set their line cursor pos, JHB Apr 2025 */
                  int i, j = 0;
                  for (i=0; i<(uApp_progress_line_cursor_pos+8)/8; i++) if (++j < (int)sizeof(leading_space_str)) strcat(leading_space_str, "\t");
                  for (i=0; i<8-(uApp_progress_line_cursor_pos % 8); i++) if (++j < (int)sizeof(leading_space_str)) strcat(leading_space_str, "\b");
               }
               else strcpy(leading_space_str, "\t\t\t\t\t\t   ");  /* if not set use previous method (approx guess) */
            }
            ...
            if (fMediaThread) {
               ...
               if (recv >= 1000000L) sprintf(&tmpstr[strlen(tmpstr)], "%sPkts %drcv", leading_space_str, recv);
               else sprintf(&tmpstr[strlen(tmpstr)], "%sPkts recv %d", leading_space_str, recv);
            }
            else sprintf(&tmpstr[strlen(tmpstr)], "%sPackets input + read %d", leading_space_str, pkt_counters[thread_index].pkt_input_cnt + pkt_counters[thread_index].pkt_read_cnt);
            ...
            if (!packet_media_thread_info[thread_index].fNoJitterBuffersUsed && (packet_media_thread_info[thread_index].packet_mode || pkt_counters[thread_index].pkt_add_to_jb_cnt)) {
               ...
               if (pkt_counters[thread_index].pkt_add_to_jb_cnt) fAnyData = true;
            }
            ...
            if (pkt_xcode_cnt || pkt_passthru_cnt) {
               ...
               if (pkt_passthru_cnt) fAnyData = true;
            }
            ...
            #else  /* it was actually printing "%s", no idea who put that in, JHB May 2025 */
            if (!fAnyData) strcat(tmpstr, "  ");  /* add some spaces for readability in case there are status messages printed by apps or libs, which is likely until at least one packet has been buffered or pulled */
            #endif
            ...
            uint32_t pkt_cnt0 = min(pkt_count[hSession0], (uint32_t)DELTA_SUM_LENGTH), pkt_cnt1 = min(pkt_count[hSession1], (uint32_t)DELTA_SUM_LENGTH), pkt_cnt2 = min(pkt_count[hSession2], (uint32_t)DELTA_SUM_LENGTH);
            ...
            sig_printf(tmpstr, PRN_LEVEL_STATS | PRN_SAME_LINE, thread_index);  /* console print */
   ...
   */
      ...
      fAllSessionsDataAvailable = true;  /* ManageSessions() sets fAllSessionsDataAvailable to false when any session is flushed by an app, indicating it has no further available data. So this will happen once in an p/m thread loop interation as one or more sessions are flushed. If an app flushes a group of sessions, then it may be false for a few iterations, but in any case it won't stay false very long */
      ...
      if (prev_thread_CPU_time) {
         ...
         if (pm_run && !packet_media_thread_info[thread_index].nChannelWavProc && packet_media_thread_info[thread_index].fPreEmptionMonitorEnabled) {  /* N channel wav file processing is post-processing, not real-time, so we don't include this in preemption checks */
            ...
            /* set preemption alarm if (i) total elapsed time - (encode + decode) is more than 1/4 the alarm time or (ii) total elapsed time is more than 3/2 the alarm time. Notes, JHB Feb 2020:
               ...
               -with -r0 mediaMin cmd line entry, which causes mediaMin to push packets as fast as possible with no timing constraints and p/m threads to pull one or more packets every iteration, pre-emption alarm is sensitve to anything, even seemingly minor, that affects per-iteration time usage. See comments near "ANALYTICS_MODE_STATUS_UPDATE_EXCEPTION" above for more info one such case, JHB Apr 2025
            */
   ...
   /* was preemption alarm set ? */
      ...
      if (fPreemptAlarm) {
         ...
         #else
         sprintf(tmpstr, "WARNING: p/m thread %d has not run for %4.2f msec, may have been preempted, num sessions = %d, fAllSessionsDataAvailable = %d, RealTimeInterval[0] = %4.2f, pm_run = %d, jb packets = %d", thread_index, 1.0*elapsed_thread_time/1000/timeScale, numSessions, fAllSessionsDataAvailable, RealTimeInterval[0], pm_run, (int)DSGetJitterBufferInfo(0, DS_JITTER_BUFFER_INFO_NUM_PKTS));
         #endif
      ...
      if (!fPreemptAlarm && packet_media_thread_info[thread_index].fProfilingEnabled) {
         ...
         packet_media_thread_info[thread_index].manage_sessions_time_max = max(packet_media_thread_info[thread_index].manage_sessions_time_max, (uint64_t)(end_profile_time - start_profile_time));
   ...
   */
   ...
   /* for mediaTest cmd line execution, look through all input streams (e.g. pcap files), read packets, push to a general multithread queue, and let pktlib organize into session-matched queues */
      ...
      if (!fMediaThread) {
         ...
         static int push_cnt[MAX_STREAMS] = { 0 };
      ...
      */
         ...
         if (cur_time > start_time + interval_time*1000*interval_count) {  /* packet input interval has elapsed ?  (all comparisons are in usec) */
            ...
            for (j=0; j<nInFiles; j++) if (in_type[j] == PCAP) {
               ...
               pkt_len[0] = DSReadPcap(fp_in[j], 0, pkt_in_buf, NULL, link_layer_length[j], NULL, NULL, NULL, 0, NULL);
...
get_pkt_info:
   ...
   /* receive and buffer incoming packets:
      ...
      -receive packets with DSRecvPackets(), which contains packets either (i) pushed by an external app in thread execution, or (ii) pushed by code immediately above that sorts through streams specified on the cmd line (i.e. each -i cmd line option specifies a pcap file or network socket)
   ...
   */
      ...
      for (i = threadid; i < (fMediaThread ? numSessions : (int)nSessions_gbl); i += nThreads_gbl) {
         ...
         if (lib_dbg_cfg.uDebugMode & DS_ENABLE_PUSHPACKETS_ELAPSED_TIME_ALARM) {
            ...
            int64_t session_last_push_time = __sync_fetch_and_add(&last_push_time[hSession], 0);  /* note -- we use an atomic int64_t read as DSPushPackets() is an asynchronous (user) thread and might concurrently write last_push_time[], JHB Jan 2020 */
         ...
         /* if bidirectional traffic flag is set, then "look ahead" into the receive packet queue and see if we can pull packets for both termination endpoints:
            ...
            -note if the bidirectional flag is not set then avoiding look-ahead increases receive queue performance, JHB Apr 2020
          */
         ...
         */
            ...
            #else
            hSession_param = (session_uFlags[hSession] & DS_SESSION_USER_MANAGED) ? hSession : -1;
            #endif

            pkt_ptr = pkt_in_buf;
            numStreams = 0;
            numPkts_matched = 0;
            unsigned int uFlags_pkt_info = DS_BUFFER_PKT_IP_PACKET | DS_PKTLIB_SUPPRESS_WARNING_ERROR_MSG;

            for (j=0; j<numPkts; j++) {  /* find channels matching this session */

               #ifdef TEST_EXTENDED_CHAN_SEARCH
               extern SESSION_CONTROL sessions[];
               printf("\n *** read packets loop before chnum parent, j = %d, numPkts = %d, sessions[%d].uFlags = 0x%x \n", j, numPkts, hSession, sessions[hSession].uFlags);
               if (hSession_param >= 0) hSession_param = -1;  /* give non user-managed value */
               else {};  /* give user managed value */
               ...
               #ifdef TEST_EXTENDED_CHAN_SEARCH
               extern SESSION_CONTROL sessions[];
               printf("\n *** read packets loop after chnum parent, chnums_lookahead[%d] = %d, chnum[0] = %d \n", j, chnums_lookahead[j], chnums_lookahead[j] >= 0 ? chnums[0] : -1);
               #endif
               ...
               pkt_ptr += pkt_len[j];  /* advance to next packet */
            }

  #if 0
  static bool fOnce2[MAX_PKTMEDIA_THREADS][8] = {{ false }};
  if (i < 8 && !fOnce2[thread_index][i]) {
     sprintf(tmpstr, "look ahead pkts = %d, time = %llu, chnums[0] = %d, chnums[1] = %d, hSession = %d, term uFlags = 0x%x\n", numPkts, session_info_thread[hSession].look_ahead_time, chnums_lookahead[0], chnums_lookahead[1], hSession, term_uFlags[hSession][0]);
     sig_printf(tmpstr, PRN_LEVEL_INFO, thread_index);
     fOnce2[thread_index][i] = true;
         ...
         */
            ...
            if (isMasterThread(thread_index)) uQueueRead ^= 1;

            #if 1
            static bool fOnce[MAX_PKTMEDIA_THREADS] = { false };
            if (!fOnce[i]) {  /* i is thread loop index */
               #ifndef __LIBRARYMODE__
               if (!fMediaThread) printf("pm/thread %d: numStreams = %d, numSessions = %d, nSessions_gbl = %d, numPkts = %d, nInFiles = %d\n", i, numStreams, numSessions, nSessions_gbl, numPkts, nInFiles);
               else
               #endif
               sprintf(tmpstr, "p/m thread %d: numStreams = %d, numSessions = %d, numPkts = %d\n", i, numStreams, numSessions, numPkts);
               sig_printf(tmpstr, PRN_LEVEL_INFO, thread_index);
               fOnce[i] = true;
            }
            #endif
         ...
         /* input time profiling, if enabled */
            ...
            /* get parent chnum (channel number) that matches the packet. Notes, JHB Feb 2019:
               ...
               -chnums_lookahead[] and chnums[] usage saves time -- for channels we've already matched there is no need to call DSGetPacketInfo() again, JHB Jan 2020
               -note use of DS_PKT_INFO_COPY_IP_HDR_LEN_IN_PINFO to obtain the packet's IP header length when retrieving another item. This plus subsequent use of DS_PKT_INFO_USE_IP_HDR_LEN minimizes DSGetPacketInfo() overhead, JHB Dec 2024
             */
               ...
               if (j < numPkts_matched) {
                  chnum_parent = chnums_lookahead[j];  /* parent (and child if applicable) already filled in, JHB Jan 2020 */
                  chnum = chnums[j];
               }
               else {
                  ...
                  #ifdef TEST_EXTENDED_CHAN_SEARCH
                  extern SESSION_CONTROL sessions[];
                  printf("\n *** buffer packets loop before chnum parent, j = %d, numPkts = %d, numPkts_matched = %d, sessions[%d].uFlags = 0x%x \n", j, numPkts, numPkts_matched, hSession, sessions[hSession].uFlags);
                  uFlags_pkt_info &= ~DS_PKTLIB_SUPPRESS_WARNING_ERROR_MSG;  /* enable all warning/error messages */
                  #endif
                  ...
                  #ifdef TEST_EXTENDED_CHAN_SEARCH
                  printf("\n *** buffer packets loop after chnum parent, chnum_parent = %d, chnum[1] = %d \n", chnum_parent, chnum_parent >= 0 ? chnums[0] : -1);
                  #endif
               }
...
debug:
               ...
               if (numPkts > 0 && isRTCPPacket(rtp_pyld_type)) {
               ...
               if (numPkts == 0) continue;  /* no packets, goto next session */
#endif
               pkt_counters[thread_index].pkt_read_cnt++;  /* increment read count */

               if (isRTCPPacket(rtp_pyld_type)) continue;  /* filter out RTCP packets, although this has probably been done by app or in DSReceivePackets() depending on uFlags. Add other filters here. The continue statement moves to the next packet. isRTCPPacket() macro is defined in pktlib.h */

               #ifdef OVERWRITE_INPUT_DATA
               if (fReuseInputs && !ReuseInputs(pkt_ptr, pkt_len[j], hSession, &session_data_t[i])) continue;
               #endif

               #ifdef ALLOW_BACKGROUND_PROCESS  /* default is to use APIs to manage packets */
               if (!use_bkgnd_process)
               #endif
               {
               ...
               */
                ...
                /* In packet mode, DSBufferPackets() adds packets to a jitter buffer. Packet mode notes:
                    ...
                    3) For pcap input, DS_BUFFER_PKT_DISABLE_OVERRUN can be set to disable jitter buffer overrun detection, which occurs when the RTP timestamp window is larger than the max delay. This effectively removes jitter buffer max depth. This flag is typically not used for live network input
                  ...
                  In frame mode, DSBufferPackets() is not called, and payloads are extracted from packets in whatever order and format in which they are read from pcap or received from network socket. Frame mode notes:
                    ...
                    1) DTX, out-of-order packets, RFC7198, and SSRC changes are not handled. If packet input is not a sequential, non-duplicated, single RTP stream representation of original encoded audio frame data prior to network transmission, then resulting decoded audio may contain artifacts or even be completely unintelligible
                ...
                */
                  ...
                  /* Session handle notes for DSBufferPackets():
                       ...
                       1) -1 tells DSBufferPackets() to match incoming packets with previously defined sessions using a hash method (based on src/dst IP addr:port values), then add the packets to the jitter buffer for that session

                       2) A specific session handle adds packets only for that session

                       3) If the session was created with the DS_SESSION_USER_MANAGED flag, then the session handle _must_ specify the correct session, otherwise the packet won't be added to the buffer. Situations where DS_SESSION_USER_MANAGED can be useful include creation of sessions with partial or all duplication of termN IP addr:port values with other sessions
                  ...
                  */
                     ...
                     if (session_info_thread[hSession].fUseJitterBuffer) {
                        ...
                        uFlags_buffer_pkts = DS_BUFFER_PKT_IP_PACKET | DS_BUFFER_PKT_DISABLE_PROBATION;  /* standard IP packet */
                        uFlags_buffer_pkts |= DS_BUFFER_PKT_EXTENDED_SESSION_SEARCH;  /* enable extended session search to allow matching of packets to mix of user-managed sessions and process-managed sessions, JHB Dec 2024 */
                        ...
                        uFlags_buffer_pkts |= DS_BUFFER_PKT_ENABLE_RFC7198_DEDUP;
                        #endif

                     /* for DTX handling with possible pcap input, we remove jitter buffer depth limit. For real-time traffic, this is not needed. For pcap input packets can be input faster than real-time and the buffer may contain a large range of timestamps at any given time, and has to handle a wider range of possible packet problems (gaps, timestamp jumps, large re-orderings, etc) that would not ordinarily occur in normal network socket input. If for any reason buffer depth must be strictly limited, this flag should not be set */

                        #if 0  /* disabling DTX is failing in analytics mode. Need to ask Chris to look into it, JHB Oct 2018 */
                        if (session_info_thread[hSession].fDTXEnabled)
                        #endif
                        {
                           uFlags_buffer_pkts |= DS_BUFFER_PKT_ALLOW_DYNAMIC_DEPTH;
                        }

                        int session_state = DSGetSessionInfo(hSession, DS_SESSION_INFO_HANDLE | DS_SESSION_INFO_STATE, 0, NULL);
                        /*if (session_state & DS_SESSION_STATE_ALLOW_TIMSTAMP_JUMP)*/ uFlags_buffer_pkts |= DS_BUFFER_PKT_ALLOW_TIMESTAMP_JUMP;  /* use this flag to ignore large positive timestamp jumps, for example long pauses in streams due to call waiting or on-hold, manual pcap manipulation, etc */
                        if (session_state & DS_SESSION_STATE_ALLOW_DYNAMIC_ADJUST) uFlags_buffer_pkts |= DS_BUFFER_PKT_ENABLE_DYNAMIC_ADJUST;  /* use this flag to enable dynamic adjust of target delay, based on incoming packets */

                        unsigned int uTermFlags = term_uFlags[hSession][term];
                        if (fTermFlagsUpdate[hSession]) uTermFlags = DSGetSessionInfo(hSession, DS_SESSION_INFO_HANDLE | DS_SESSION_INFO_TERM_FLAGS, term+1, NULL);  /* allow for possibility that ManageSessions() has not yet updated term_uFlags[], JHB Jul 2025 */
                        if (uTermFlags & TERM_EXCLUDE_RTP_PAYLOAD_TYPE) uFlags_buffer_pkts |= DS_BUFFER_PKT_EXCLUDE_PAYLOAD_TYPE;  /* exclude RTP payload type from packet validation, JHB Jul 2025 */
  ...
  log_pkt_in_index++;
   ...
   //#define DEBUG_TELECOM_MODE_TIMESTAMP_GAP
                        ...
                        if (ret_val > 0) {
                           ...
                           if (lib_dbg_cfg.uPktStatsLogging & DS_ENABLE_PACKET_TIME_STATS && !fPreemptAlarm) RecordPacketTimeStats(chnum, pkt_ptr, packet_len[0], pkt_info[0], pkt_count[hSession], PACKET_TIME_STATS_INPUT);
                           #endif

                        /* mark session's ssrc state as live upon buffering a packet, record channel's most recent buffer time */

                 //   if (session_info_thread[hSession].ssrc_state[term] == SSRC_DORMANT) printf("\n *** session %d term %d setting from dormant to live \n", hSession, term);
                           if (!nDormantChanFlush[hSession][term])
                             session_info_thread[hSession].ssrc_state[term] = SSRC_LIVE;
                        ...
                        /* Act on error and/or warning conditions from DSBufferPackets(). Notes:
                           ...
                           1) DSBufferPackets returns -1 for error conditions and the number of successfully packets added for warning conditions, which requires checking the API's most recent status to know if anything went wrong. Warning conditions are defined as one or more packets to be added, but not all, have some issue that prevents them from being added. An example might be a bit error that causes header validation to fail

                           2) Which warning conditions to act on is application-dependent. In this case we treat RTP validation as an error situation, since if one packet from a pcap RTP stream is wrong, then likely all packets for that stream have the same problem (other RTP streams in the pcap may be fine, which this example handles). For live input that might not be the case, so this is just an example
                         */
                     ...
                     if (session_info_thread[hSession].fUseJitterBuffer && (uFlags_buffer_pkts & DS_PKTLIB_HOST_BYTE_ORDER)) uFlags_info |= DS_PKTLIB_HOST_BYTE_ORDER;  /* if user app packet headers are in host byte order, then adjust uFlags for subsequent DSGetPacketInfo() calls. mediaTest doesn't do this, so this line is here just as an example */
                  }
                  else {  /* frame mode */
                  ...
                  #else
                  if (ret_val > 0 && (nSSRC_change = CheckForSSRCChange(hSession, &chnum_parent, pkt_ptr, &pkt_len[j], 1, uFlags_info, session_uFlags[hSession], pkt_ctrs, thread_index)) > 0) {  /* look for an SSRC change in the input stream, set fSSRC_Change[] if found */
                  #endif
                     ...
                     /* the current thinking is no action is needed here, as both parent and child will be acted on by continued calls to DSGetOrderedPackets(), and the parent jitter buffer will end up fully flushed, JHB Jan 2020:
                  ...
                  if ((uPktStatsLogging = DSIsPktStatsHistoryLoggingEnabled(thread_index))) {
                     ...
                     /* add packet stats entry */
                        ...
                        int num_stats = DSPktStatsAddEntries(input_pkts[chnum].pkt_stats, uFlags_info, ret_val >= 0 ? ret_val : 1, pkt_ptr, &pkt_len[j], &pkt_info[j]);  /* log input packets; for multiple input streams the DS_PKTSTATS_LOG_COLLATE_STREAMS flag is used (see below) */
                        if (num_stats > 0) pkt_counters[thread_index].num_input_pkts += num_stats;

                        manage_pkt_stats_mem(input_pkts, chnum, num_stats);
                  #else

                  if (isMasterThread(thread_index) && (uPktStatsLogging = DSIsPktStatsHistoryLoggingEnabled(thread_index))) {
                     ...
                     /* fill in session and stream group info. Note that group index (idx) may be -1 if session does not belong to a stream group, and if so we fill this in so it can be later referenced by packet stats processing JHB Dec2019 */
                        ...
                        input_pkts[pkt_counters[thread_index].num_input_pkts].idx = DSGetStreamGroupInfo(chnum, DS_STREAMGROUP_INFO_HANDLE_CHNUM, NULL, NULL, NULL);  /* returns -1 if chnum is not a stream group member */

                     /* add packet stats entry */

                        int num_stats = DSPktStatsAddEntries(&input_pkts[pkt_counters[thread_index].num_input_pkts], uFlags_info, ret_val >= 0 ? ret_val : 1, pkt_ptr, &pkt_len[j], &pkt_info[j]);  /* log input packets; for multiple input streams the DS_PKTSTATS_LOG_COLLATE_STREAMS flag is used (see below) */
                        if (num_stats > 0) pkt_counters[thread_index].num_input_pkts += num_stats;
      ...
      if (!fPreemptAlarm && packet_media_thread_info[thread_index].fProfilingEnabled) {
         ...
         if (input_time > 0) {
            ...
            packet_media_thread_info[thread_index].input_time_max = max(packet_media_thread_info[thread_index].input_time_max, (uint64_t)input_time);
         ...
         if (buffer_time > 0) {
            ...
            packet_media_thread_info[thread_index].buffer_time_max = max(packet_media_thread_info[thread_index].buffer_time_max, (uint64_t)buffer_time);
            ...
            if (no_pkt_elapsed_time_thread > pktlib_gbl_cfg.uThreadEnergySaverInactivityTime*1000) {  /* comparison is in usec */
               ...
               if (pktlib_gbl_cfg.uThreadEnergySaverWaitForAppQueuesEmptyTime &&
                  ...
                  for (i = threadid; i < (fMediaThread ? numSessions : (int)nSessions_gbl); i += nThreads_gbl) {
                     ...
                     if (DSPullPackets(DS_PULLPACKETS_GET_QUEUE_STATUS | DS_PULLPACKETS_OUTPUT | DS_PULLPACKETS_JITTER_BUFFER, NULL, NULL, hSession, NULL, 0, 0) == 0) {
   ...
   /* incoming packets -- first look for available network socket packets, if UDP socket input is enabled */
      ...
      if (recv_len > 0)
      {
         ...
         if (packet_length > 0) {
            ...
            ret_val = DSBufferPackets(-1, DS_BUFFER_PKT_IP_PACKET, pkt_in_buf, packet_len, &pkt_info[0], NULL, cur_time);
         ...
         if ((uPktStatsLogging = DSIsPktStatsHistoryLoggingEnabled(thread_index))) {
            ...
            /* fill in session and stream group info. Note that group index (idx) may be -1 if session does not belong to a stream group, JHB Dec 2019 */
            ...
            /* add packet stats entry */
               ...
               int num_stats = DSPktStatsAddEntries(input_pkts[NCORECHAN].pkt_stats, DS_BUFFER_PKT_IP_PACKET, ret_val >= 0 ? ret_val : 1, pkt_in_buf, packet_len, &pkt_info[0]);
         ...
         if (isMasterThread(thread_index) && (uPktStatsLogging = DSIsPktStatsHistoryLoggingEnabled(thread_index))) {
            ...
            /* add packet stats entry */
               ...
               int num_stats = DSPktStatsAddEntries(&input_pkts[pkt_counters[thread_index].num_input_pkts], DS_BUFFER_PKT_IP_PACKET, ret_val >= 0 ? ret_val : 1, pkt_in_buf, packet_len, &pkt_info[0]);
               if (num_stats > 0) pkt_counters[thread_index].num_input_pkts += num_stats;
         ...
         */
            ...
            do {
               ...
               /* note:  n does not correspond to term-1, for example session term1 may be inactive and term2 active, in which case n = 0 and term-1 = 1. This is why we call DSGetSessionInfo() with DS_SESSION_INFO_TERM, JHB Sep2019 */
                  ...
                  #if 0  /* move this to buffer add side so that session's ssrc state is marked as live when a packet is buffered w/o error. This is (i) independent of jitter buffer target delay (for example a short session that never primes, or takes ) and (ii) compatible with both analytics and telecom modes, JHB Apr 2020 */
                  ...
                  if (packet_media_thread_info[thread_index].packet_mode) {  /* packet mode */
                     ...
                     */
                        ...
                        if (!fOnce[hSession][term]) {
                           ...
                           unsigned int uTermFlags = DSGetSessionInfo(hSession, DS_SESSION_INFO_HANDLE | DS_SESSION_INFO_TERM_FLAGS, term+1, NULL);
                     ...
                     */
                     ...
                     /* Enable DTMF event handling in DSGetOrderedPackets(). See the check below for pkt_info[] = DS_PKT_PYLD_CONTENT_DTMF */
                     ...
                     /* enable timestamp gap resync in telecom mode for large gaps under specific conditions. Notes JHB Sep 2023:
                        ...
                        -this is now enabled in non-library mode (mediaTest cmd line mode); DSGetSessionInfo(ch, DS_SESSION_INFO_CHNUM | DS_SESSION_INFO_RTP_CLOCKRATE ...) added to support this, JHB Apr 2024
                     */
...
pull:
                        ...
                        num_pkts += (pull_pkts = DSGetOrderedPackets(hSession, uFlags_get | DS_GETORD_PKT_SESSION, cur_time, pkt_ptr + ptr_ofs, &packet_len[num_pkts], &pkt_info[num_pkts], &uInfo));
  ...
  if (num_pkts > 0) { count[chan_nums[n]] += num_pkts; printf("\n *** pull[%d] = %d, count[%d] = %d \n", chan_nums[n], num_pkts, chan_nums[n], count[chan_nums[n]]); }
                     ...
                     /* record last pull time for this channel */
                        ...
                        else if (num_pkts) last_pull_time[chan_nums[n]] = cur_time;  /* in telecom mode we go by whether a packet was returned, JHB Apr 2020 */
                        ...
                        if (num_pkts == 0 && session_info_thread[hSession].fDataAvailable) {
                           ...
                           if ((uFlags_get & DS_GETORD_PKT_ANALYTICS) || pkt_info[0] == DS_PKT_PYLD_CONTENT_PROBATION) {  /* look for special case where packets exist but are available yet because nominal jitter buffer delay has not been reached (jitter buffer not yet "primed") */

                              pkt_info[0] = DS_PKT_PYLD_CONTENT_PROBATION;
                              num_pkts = 1;
                           }
                        }
#endif
   ...
   if (num_pkts > 0) {
     int k;
     char tmpstr[1000];
     sprintf(tmpstr, "RTP seq num, num_pkts = %d", num_pkts);
     uint8_t* p = pkt_ptr;
     ...
     printf("\n *** %s \n", tmpstr);
                     ...
                     else {  /* jitter buffer not used, set num_pkts and pkt_ptr; packet_len[] and pkt_info[] remain set from input stream loop above */

                        num_pkts = pkts_read[stream_indexes[n]];
                        pkt_ptr = pkt_in_buf;
                     }
                  }
                  else {  /* frame mode, set num_pkts and pkt_ptr; packet_len[] and pkt_info[] are not used in frame mode */
               ...
               /* note:  from this point forward, if jitter buffer is not used then pkt_ptr, packet_len, pkt_info, and num_pkts all refer to pcap or input stream values */

                  int media_data_len = 0;  /* length of decoded media frame, in bytes */
                  HCODEC hCodec = (intptr_t)NULL;
                  int prev_chnum = -1, in_media_sample_rate __attribute__ ((unused)) = 0;
               ...
               /* packet payload processing loop. A jitter buffer may have returned multiple packets representing multiple channels */
                  ...
                  for (j=0; j<num_pkts; j++) {
                     ...
                     int rtp_pyld_len = PktInfo.rtp_pyld_len;
                     #endif
                  ...
                  /* find the channel number matching the buffer output packet (could be parent or child), and error check */
                     ...
                     #endif
                     {
                        ...
                        break;  /* break out of packet loop, we could continue also but I guess the assumption is if one packet is not matching due to some unforeseen error scenario others won't either */
                     }
                     ...
                     if (pkt_info[j] != DS_PKT_PYLD_CONTENT_PROBATION) {
                        ...
                        if (packet_media_thread_info[thread_index].packet_mode && session_info_thread[hSession].fUseJitterBuffer) {
                           ...
                           #ifdef PACKET_TIME_STATS
                           if (lib_dbg_cfg.uPktStatsLogging & DS_ENABLE_PACKET_TIME_STATS && !fPreemptAlarm) RecordPacketTimeStats(chnum, pkt_ptr, packet_len[j], pkt_info[0], 0, PACKET_TIME_STATS_PULL);  /* pkt_count is not used with PACKET_TIME_STATS_PULL */
                           #endif
                           ...
                           /* add packet stats entry */
                              ...
                              int num_stats = DSPktStatsAddEntries(pulled_pkts[chnum].pkt_stats, uFlags_info, 1, pkt_ptr, &packet_len[j], &pkt_info[j]);  /* we log all buffer output packets; for multiple output streams the DS_PKTSTATS_LOG_COLLATE_STREAMS flag is used (see below) */
                           ...
                           if (isMasterThread(thread_index) && DSIsPktStatsHistoryLoggingEnabled(thread_index)) {

                           /* fill in session and stream group info. Note that group index (idx) may be -1 if session does not belong to a stream group, and if so we fill this in so it can be later referenced by packet stats processing, JHB Dec2019 */

                              pulled_pkts[pkt_counters[thread_index].num_pulled_pkts].chnum = chnum;
                              pulled_pkts[pkt_counters[thread_index].num_pulled_pkts].idx = DSGetStreamGroupInfo(chnum, DS_STREAMGROUP_INFO_HANDLE_CHNUM, NULL, NULL, NULL);  /* returns -1 if chnum not a stream group member */

                           /* add packet stats entry */

                              int num_stats = DSPktStatsAddEntries(&pulled_pkts[pkt_counters[thread_index].num_pulled_pkts], uFlags_info, 1, pkt_ptr, &packet_len[j], &pkt_info[j]);  /* we log all buffer output packets; for multiple output streams the DS_PKTSTATS_LOG_COLLATE_STREAMS flag is used (see below) */
                              if (num_stats > 0) pkt_counters[thread_index].num_pulled_pkts += num_stats;
            ...
            //if (chnum == 0 && ++SentCount >= 1306) printf("\n *** jb sent packet #%d \n", SentCount);  
                              ...
                              rtp_pyld_type[j] = PktInfo.rtp_pyld_type;
                              #endif
                           }
                        }
            ...
            //if (chnum == 0 && SentCount >= 1306) printf("\n *** checking for pkt content #%d, content = 0x%x \n", SentCount, pkt_info[j]);
                  ...
                  /* check if packet is DTMF event */
                     ...
                     if (pkt_info[j] == DS_PKT_PYLD_CONTENT_DTMF || pkt_info[j] == DS_PKT_PYLD_CONTENT_DTMF_SESSION) {  /* check for DTMF packets, both generic and matching session-defined DTMF payload types, JHB May 2019 */

                        int dtmf_display_msg_limit;

                        if (termInfo_link.voice.dtmf_mode & DS_DTMF_RTP) {
                        ...
                        DSGetDTMFInfo(-1, (uintptr_t)NULL, rtp_pyld_ptr, rtp_pyld_len, &dtmf_info);
            ...
            //if (chnum == 0 && SentCount >= 1306) printf("\n *** inside media packet #%d \n", SentCount);
                     ...
                     /* find the channel's codec instance (channel can be parent or child). Notes: 1) if channel has changed from previous loop iteration or not yet initialized (e.g. first packet is a DTMF event, followed by a media packet) then we need to look up hCodec, 2) decoder and encoder instances are always created for a session, even if they are not used (for example, for passthru or unidirectional operation, both decoder and encoder instances exist) */
                        ...
                        if ((chnum != prev_chnum || hCodec == (intptr_t)NULL) && (hCodec = DSGetSessionInfo(chnum, DS_SESSION_INFO_CHNUM | DS_SESSION_INFO_CODEC, 1, &termInfo)) < 0) {

                           Log_RT(2, "ERROR: pkt/media thread %d for %s packet says DSGetSessionInfo() failed to get decode codec info, hSession = %d, chnum = %d \n", thread_index, packet_type == MEDIA_PACKET ? "media" : "probation", hSession, chnum);
                           break;
                        }
                        ...
                        /* decode one or more frames inside an RTP payload extracted from either (i) packet pulled from jitter buffer (packet mode) or (ii) input packet (frame mode). Notes:

                           -the return value from DSCodecDecode() for decoded media length handles cases where packets contain more than one payload. DSGetCodecInfo() could also be used to get the raw audio frame length but not robust for actual packet flow
                           -both input payload length and output media buffer length are in bytes
                           -for pass-thru case, DSCodecDecode() simply copies data from input to output buffer
                        */
         ...
         */
   static int rtp_pyld_len_save, nOnce = 0;
   static uint8_t rtp_pyld_save[200];

   if (termInfo.codec_type == DS_CODEC_VOICE_EVS) {

      if (nOnce == 0) {
         memcpy(rtp_pyld_save, rtp_pyld_ptr, rtp_pyld_len);
         rtp_pyld_len_save = rtp_pyld_len;
      }

      if (nOnce == 5) {  /* overwrite frame 5 with frame 0 */
         memcpy(rtp_pyld_ptr, rtp_pyld_save, pyld_len_save);
         rtp_pyld_len = rtp_pyld_len_save;
         printf("\n === EVS frame overwrite \n");
      }
      ...
      nOnce++;
                           ...
                           if (isVoiceCodec(termInfo.codec_type) || isAudioCodec(termInfo.codec_type)) {  /* voice or audio (isXxxCodec() macros are in shared_include/codec.h) */
                              ...
                              memcpy(media_data_buffer, rtp_pyld_ptr, rtp_pyld_len);
                              media_data_len = rtp_pyld_len;
                           }
   ...
   if (chnum == 4) {  /* note: set channel number(s) here to control debug range */
      int i, sum = 0, sum2 = 0;
      for (i=0; i<rtp_pyld_len; i++) sum += rtp_pyld_ptr[i];
      for (i=0; i<media_data_len; i++) sum2 += media_data_buffer[i];

      sprintf(tstr, "pkt\t%d\t\t\t\t%d\t\t\t\t%d\t\t\t\t%d\t\t\t\t%d\t\t\t\t%d\n", log_pkt_index, rtp_pyld_len, media_data_len, sum, sum2, hCodec);
      fwrite(tstr, strlen(tstr), 1, fp_log_pkt);
      log_pkt_index++;
   }
   #endif

                           #ifdef VIDEO_DATA_FLOW_DEBUG
                           static bool fOnce[16] = { false };
                           if (isVideoCodec(termInfo.codec_type) && !fOnce[hSession]) { fOnce[hSession] = true; printf("\n *** video decode pass-thru hSession = %d, rtp_pyld_len = %d, media_data_len = %d \n", hSession, rtp_pyld_len, media_data_len); }
                           #endif

                           if (isEVSCodec(termInfo.codec_type) || isAMRCodec(termInfo.codec_type)) {  /* handle return data, JHB Oct 2022. AMR support added Dec 2024 */

                              int bitrate_index = OutArgs.bitRate > 0 ? DSGetCodecInfo(termInfo.codec_type, DS_CODEC_INFO_BITRATE_TO_INDEX, OutArgs.bitRate, 0, NULL) : -1;  /* convert bitrate to an index used by all codecs. Note that to avoid warning messages bitrate param should be checked for > 0 in case codec doesn't know it, for example input frame is a NO_DATA type, JHB Nov 2024 */
                           ...
                           /* add audio classification to current list */

                              if (OutArgs.bitRate >= 0) DSSetJitterBufferInfo(chnum, DS_JITTER_BUFFER_INFO_PKT_CLASSIFICATION_LIST, OutArgs.frameType | DSGetJitterBufferInfo(chnum, DS_JITTER_BUFFER_INFO_PKT_CLASSIFICATION_LIST));
                           }

                           if (media_data_len < 0) {

                              Log_RT(2, "ERROR: pkt/media thread %d says DSCodecDecode() returned error condition %d/%d packets from jitter buffer, hSession = %d, chnum = %d, rtp_pyld_len = %d, hCodec = %d \n", thread_index, j+1, num_pkts, hSession, chnum, rtp_pyld_len, hCodec);
                              goto next_packet;  /* error condition */
                           }
                        }
                        ...
                        if (termInfo.codec_type != DS_CODEC_NONE) {  /* check for term1 (decoder) pass-thru (codec type constants are in session.h) */
                     ...
                     /* in mediaTest mode, write decoded audio data to wav file, if specified in the cmd line */
                     ...
                     chnum_parent = chan_nums[n];  /* for time being we store audio stream data using only parent chnum. DSGetStreamData() would need to be modified to search its chnum param for child channels, JHB Dec 2019 */
                     #endif
   ...
   for (l=0; l<len; l++) y[l] = amp*sin(2*M_PI*l/len);
                        ...
                        #ifdef DEBUG_TELECOM_MODE_TIMESTAMP_GAP
                        static int nCount = 0;
                        if (nCount < 200 && chnum_parent == 4) Log_RT(4, " *** [%d] DSStoreStreamData() ch %d, j = %d, num_pkts = %d, pyld_len = %d, media_data_len = %d \n", nCount++, chnum_parent, j, num_pkts, pyld_len, media_data_len);
                        #endif

                        #ifdef VIDEO_DATA_FLOW_DEBUG
                        static int nCount[16] = { 0 };
                        if (isVideoCodec(termInfo.codec_type) && nCount[hSession] < num_pkts) { nCount[hSession]++; printf("\n *** video before store stream hSession = %d, num_count = %d, rtp_pyld_len = %d, media_data_len = %d \n", hSession, nCount[hSession], rtp_pyld_len, media_data_len); }
                        #endif

                        #if 0  /* enable to see length values after transcoding (ratio = out_media_data_len/data_length), JHB Nov 2023 */
                           static bool fOnce[16] = { false };
...
next_packet:
                  ...
                  /* error out if we can't fit the next packet, JHB Apr 2020 */
...
//                  uint8_t stream_data[5*10240];
                  ...
                  int pyld_len_encode = 0, out_media_data_len = 0, out_media_sample_rate __attribute__ ((unused)) = 0;
            ...
            //if (chnum == 0 && SentCount >= 1306 && num_data) printf("\n *** after get stream #%d, num_data = %d \n", SentCount, num_data);
                  ...
                  #ifdef VIDEO_DATA_FLOW_DEBUG
                  static bool fOnce[16] = { false };
                  if (isVideoCodec(termInfo.codec_type) && !fOnce[hSession] && num_data) {
                     int i; fOnce[hSession] = true; char tmpstr[400];
                     sprintf(tmpstr, "\n *** video after get stream hSession = %d, num_data = %d", hSession, num_data);
                     for (i=0; i<num_data; i++) sprintf(&tmpstr[strlen(tmpstr)], " pDataLen[%d] = %d, pDataChan[%d] = %d", i, pDataLen[i], i, pDataChan[i]);
                     printf("%s \n", tmpstr);
                  }
                  #endif
               ...
               */
                  ...
                  for (j=0; j<num_data; j++) {  /* start of payload processing loop */
                     ...
                     chnum = pDataChan[j];
                     data_length = pDataLen[j];

                     if (j > 0) stream_ptr += pDataLen[j-1];

                     if (pDataInfo[j] == DS_PKT_PYLD_CONTENT_MEDIA || pDataInfo[j] == DS_PKT_PYLD_CONTENT_MEDIA_REUSE) packet_type = MEDIA_PACKET;  /* bug-fix: also check for DS_PKT_PYLD_CONTENT_MEDIA_REUSE, JHB Nov 2024 */
                     else if (pDataInfo[j] == DS_PKT_PYLD_CONTENT_DTMF) packet_type = DTMF_PACKET;

                     else {
                        Log_RT(1, "CRITICAL: packet/media thread %d says invalid data type from DSGetStreamData(): 0x%x\n", thread_index, pDataInfo[j]);
                        break;
                     }

                     #else  /* if DECOUPLE_STREAM_PROCESSING not defined */
                  ...
                  /* Note -- for combined packet/payload + stream processing, chnum, rtp_pyld_ptr, and pyld_len values set earlier may be used */
                     ...
                     if (packet_type == MEDIA_PACKET) {
                        ...
                        data_length = out_media_data_len;  /* out_media_data_len is uninitialized, need to fix at some point. Not an issue when DECOUPLE_STREAM_PROCESSING is defined */
                     }
                     else if (packet_type == DTMF_PACKET) {
                     ...
                     if (chnum != prev_chnum) {  /* update hCodec, hCodec_link, termInfo, and termInfo_link if chnum changes, JHB Feb 2020 (hCodec and termInfo moved here, JHB Sep 2024) */

                        int term = 1;

                        hCodec = DSGetSessionInfo(chnum, DS_SESSION_INFO_CHNUM | DS_SESSION_INFO_CODEC, term, &termInfo);  /* 1 indicates incoming (encode) codec handle and type (type stored in termInfo) */

                        if (hCodec > 0) { term = 2; hCodec_link = DSGetSessionInfo(chnum, DS_SESSION_INFO_CHNUM | DS_SESSION_INFO_CODEC, term, &termInfo_link); }  /* 2 indicates outgoing (encode) codec handle and type (type stored in termInfo_link) */

                        if (hCodec < 0 || hCodec_link < 0) {

                           Log_RT(1, "CRITICAL: packet/media thread %d says failed to get hCodec and terminfo for term %d, chnum = %d \n", thread_index, term, chnum);
                           break;  /* exit num_data loop (result of DSGetStreamData()) */
                        }
                     }

                     bool fOutputPacketProcessing = !(term_uFlags[hSession][0] & TERM_DISABLE_OUTPUT_QUEUE_PACKETS);  /* see if output packet queueing is disabled by application (isXxxCodec() macros are in shared_include/codec.h), JHB Sep 2024 */
                     bool fTimestampMatchModeL16Transcode = (uTimestampMatchMode & TIMESTAMP_MATCH_MODE_ENABLE) && packet_type == MEDIA_PACKET;  /* timestamp match mode needs most of output packet processing, but not all, so we use 2 flags */
                     ...
                     /* if first time through the num_data loop, or channel changes between data items, then refresh chnum_parent and fStreamGroupMember */
...
//     for (l=0; l<len; l++) y[l] = amp*sin(2*M_PI*l/len);
     count++;
                           ...
                           if ((ret_val = DSStoreStreamGroupContributorData(chnum_parent, stream_ptr, data_length, 0)) < 0) {
                              ...
                              int pull_queue_level = DSPullPackets(DS_PULLPACKETS_GET_QUEUE_LEVEL | DS_PULLPACKETS_OUTPUT, NULL, NULL, hSession, NULL, 0, 0);
                     ...
                     /* audio transcode: convert sampling rate (if needed) and encode. Additional notes, JHB Sep 2024:
                     
                        -first check (i) packet output to application queue or (ii) timestamp match mode transcoding to L16 raw audio
                        -video codec bitstream output omits transcoding but includes other output packet processing 
                        -Timestamp match mode requires most of transcoding process (except for DSSendPacket() queueing if disabled by TERM_DISABLE_OUTPUT_QUEUE_PACKETS)
                     */

                        if (fOutputPacketProcessing || fTimestampMatchModeL16Transcode) {
  \
                           if (isVoiceCodec(termInfo.codec_type) || isAudioCodec(termInfo.codec_type)) {  /* voice or audio */

                           /* perform sampling rate conversion (if needed for encoding to output endpoint). Audio data is not touched if input and output sampling rates are the same for this channel (as defined in its session config info) */
                              ...
                              static bool fOnce[16] = { false };
                              if (!fOnce[chnum]) { fOnce[chnum] = true; printf("\n *** after Fs conversion ch = %d, out_media_data_len = %d, data_length = %d \n", chnum, out_media_data_len, data_length); }
                              #endif
                           ...
                           /* encode one or more raw audio frames. Notes:
                              ...
                              Additional notes JHB Aug 2023:

                              -currently we transcode to G711 and sending packets to app threads with DSSendPackets(... DS_PULLPACKETS_OUTPUT ...)
                           ...
                           */
                              ...
                              #if 0  /* enable to see length values after transcoding (ratio = out_media_data_len/data_length), JHB Nov 2023 */
                              static bool fOnce[16] = { false };
                              if (!fOnce[chnum]) { fOnce[chnum] = true; printf("\n *** after Fs conversion and encoding ch = %d, out_media_data_len = %d, data_length = %d, pyld_len_encode = %d, encoder codec type = %d \n", chnum, out_media_data_len, data_length, pyld_len_encode, termInfo_link.codec_type); }
                              #endif

                              if ((int)pyld_len_encode < 0) break;  /* on error condition break out of payload processing loop */

                              if (termInfo_link.codec_type != DS_CODEC_NONE) {  /* check for term2 (encoder) pass-thru (codec type constants are in session.h) */
                           ...
                           else if (isVideoCodec(termInfo.codec_type)) {  /* parse video codec packets into .h26x or .hevc bitstream (isXxxCodec() macros are in shared_include/codec.h), JHB Sep 2024  */

                              //pyld_len_encode = parse_H26x_bitstream();
                              memcpy(encoded_data_buffer, stream_ptr, data_length);
                              pyld_len_encode = data_length;
                              pkt_bitstream_cnt++;

                              #ifdef VIDEO_DATA_FLOW_DEBUG
                              static bool fOnce[16] = { false };
                              if (isVideoCodec(termInfo.codec_type) && !fOnce[hSession]) { fOnce[hSession] = true; printf("\n *** video encode pass-thru hSession = %d, pyld_len_encode = %d \n", hSession, pyld_len_encode); }
                              #endif
                           }
                        }

                     }  /* packet_type == MEDIA_PACKET */

                  /* format transcoded packet, send to application output packet queue */

                     if (fOutputPacketProcessing || fTimestampMatchModeL16Transcode) {

                        uFlags_format = 0;

                        if (termInfo_link.codec_type == DS_CODEC_VOICE_G711_ULAW || termInfo_link.codec_type == DS_CODEC_VOICE_G711_ALAW || termInfo_link.codec_type == DS_CODEC_AUDIO_L16) {  /* for G711U/A RTP output, control marker bits in output stream */
                        ...
                        #define USE_JITTER_BUFFER_OUTPUT_RTPTIMESTAMP  /* match timestamps of transcoded output to jitter buffer output, JHB Aug 2023 */
                           ...
                           static bool fTranscodingRTPTimestamp_warning[16] = { false };
                           if (!isVideoCodec(termInfo.codec_type) && !fTranscodingRTPTimestamp_warning[chnum & 15] && packet_type == MEDIA_PACKET && num_pkts != num_data) { fTranscodingRTPTimestamp_warning[chnum & 15] = true; Log_RT(3, "WARNING: packet/media thread %d says inside RTP timestamp matching for transcoded media, ch = %d, num pkts %d <> num_data %d, data_length = %d \n", thread_index, chnum, num_pkts, num_data, data_length); }
                           ...
                           uint8_t rtp_clockrate_encoder = DSGetSessionInfo(chnum, DS_SESSION_INFO_CHNUM | DS_SESSION_INFO_RTP_CLOCKRATE, 2, NULL);  /* use rtp_clockrate_xxx values to transrate between input and output RTP streams, instead of out_media_data_len and data_length, JHB Sep 2024 */
                           uint8_t rtp_clockrate_decoder = max(DSGetSessionInfo(chnum, DS_SESSION_INFO_CHNUM | DS_SESSION_INFO_RTP_CLOCKRATE, 1, NULL), (int64_t)1);  /* don't allow division by zero or negative number */

                           #if 0
                           static bool fOnce[64] = { false };
                           if (!fOnce[chnum]) { printf("\n *** ch %d encode RTP clock rate factor = %f, rtp_clockrate_decoder = %d, rtp_clockrate_encoder = %d \n", chnum, 1.0*rtp_clockrate_encoder/rtp_clockrate_decoder, rtp_clockrate_decoder, rtp_clockrate_encoder); fOnce[chnum] = true; }
                           #endif

                           formatPkt.rtpHeader.Timestamp = 1.0*rtp_timestamp[j]*rtp_clockrate_encoder/rtp_clockrate_decoder;  /* apply up or down RTP clock rate conversion factor, if any, JHB Feb 2024 */
                           #endif
                           formatPkt.rtpHeader.SSRC = rtp_ssrc[j];

                           if (termInfo_link.codec_type == DS_CODEC_VOICE_G711_ULAW) formatPkt.rtpHeader.PyldType = PCMU_PYLD_TYPE;
                           else if (termInfo_link.codec_type == DS_CODEC_VOICE_G711_ALAW) formatPkt.rtpHeader.PyldType = PCMA_PYLD_TYPE;
                           else if (termInfo_link.codec_type == DS_CODEC_AUDIO_L16) formatPkt.rtpHeader.PyldType = L16_PYLD_TYPE;  /* linear 16-bit PCM (i.e. not compressed) */
...
static int log_pkt_index = 0;
  ...
  log_pkt_index++;
                     ...
                     /* in packet/media thread mode send packet over network, otherwise check for associated file handle and if so write to file */
                        ...
                        if (fMediaThread) {
                           ...
                           if (fOutputPacketProcessing) pkt_counters[thread_index].pkt_write_cnt++;
                           ...
                           #if defined(ENABLE_TERM_MODE_FIELD) && defined(ENABLE_TERM_MODE_DONT_CARE)  /* not currently defined */
               ...
               /* if no contributions yet, don't allow timing internal to DSProcessStreamGroupContributors() to initialize, otherwise a stream group's first contribution can have a ptime msec wobble, JHB May 2020:
      ...
      if (!fPreemptAlarm) {
         ...
         if (decode_time > 0) {
            ...
            packet_media_thread_info[thread_index].decode_time_max = max(packet_media_thread_info[thread_index].decode_time_max, (uint64_t)decode_time);
         ...
         if (encode_time > 0) {
            ...
            packet_media_thread_info[thread_index].encode_time_max = max(packet_media_thread_info[thread_index].encode_time_max, (uint64_t)encode_time);
         ...
         if (packet_media_thread_info[thread_index].fProfilingEnabled) {
            ...
            if (chan_time > 0) {
               ...
               packet_media_thread_info[thread_index].chan_time_max = max(packet_media_thread_info[thread_index].chan_time_max, (uint64_t)chan_time);
            ...
            if (pull_time > 0) {
               ...
               packet_media_thread_info[thread_index].pull_time_max = max(packet_media_thread_info[thread_index].pull_time_max, (uint64_t)pull_time);
            ...
            if (stream_group_time > 0) {
               ...
               packet_media_thread_info[thread_index].stream_group_time_max = max(packet_media_thread_info[thread_index].stream_group_time_max, (uint64_t)stream_group_time);
...
/* close network sockets */
   ...
   if (isMasterThread(thread_index) && fNetIOAllowed) {
   ...
   if (isMasterThread(thread_index)) fPMThreadsClosing = 1;
...
sync_exit:
   ...
   if (!fSyncExit[thread_index]) {
      ...
      if (isMasterThread(thread_index) || !fMediaThread) {
...
bool fChanFound = false;
   ...
   for (int i=0; i<MAX_TERMS; i++) {
      ...
      #else
      #ifdef USE_OLD_DORMANT_DISABLE_FLAG  /* now we always detect and notify, but flush only if TERM_ENABLE_DORMANT_SESSION is set, JHB Jul 2025 */
      if (term_uFlags[hSession][i] & TERM_DISABLE_DORMANT_SESSION_DETECTION) continue;  /* go no further, hSession's channel doesn't want to be considered dormant and flushed, JHB Sep 2022 */
      #endif
      #endif
  ...
  /* skip stream if it's not yet active or if dormant session detection is disabled by termination endpoint contributor flags */
      ...
      for (int j=threadid; j<(packet_media_thread_info[thread_index].fMediaThread ? numSessions : (int)nSessions_gbl); j += nThreads_gbl) {

         hSession2 = get_session_handle(hSessions_t, j, thread_index);

         if (hSession2 >= 0 && hSession2 != hSession) for (int i2=0; i2<MAX_TERMS; i2++) {
            ...
            if (stream_ssrc == stream_ssrc2 && session_info_thread[hSession].ssrc_state[i] == SSRC_LIVE && session_info_thread[hSession2].ssrc_state[i2] == SSRC_LIVE) {  /* two streams with same SSRC, and both are live ? */
            ...
            /* check if we exceed time period before a channel SSRC can be considered dormant. Notes, JHB Sep 2022:
               ...
               -we now detect and notify on duplication and reuse of channel SSRCs, but not flush unless TERM_ENABLE_DORMANT_SESSION is set in term_uFlags[hSession][i], JHB Jul 2025
             */
               ...
               if ((int64_t)((cur_time - last_buffer_time[chnum]) - (cur_time - last_buffer_time[chnum2]))/1000 > term1.dormant_SSRC_wait_time) {  /* note we don't use timeScale here; we assume the app is also operating at accelerated time, JHB Jun 2023 */
                  ...
                  if (!nDormantChanFlush[hSession][i]) {
                     ...
                     #ifdef USE_OLD_DORMANT_DISABLE_FLAG
                     ...
                     #else  /* updated implementation notifies on duplicated and reused SSRCs, but doesn't implement dormant session functionality unless TERM_ENABLE_DORMANT_SESSION flag is set, JHB Jul 2025 */

                     if (term_uFlags[hSession][i] & TERM_ENABLE_DORMANT_SESSION) nDormantChanFlush[hSession][i] = DSGetJitterBufferInfo(chnum, DS_JITTER_BUFFER_INFO_TARGET_DELAY);  /* if there was a way to force all remaining packets out at once, that would avoid the count-down */

                     if (nSSRCDuplicationDetection[chnum2] != chnum+1) uSSRCDuplicationCount[chnum2]++;  /* update stats */
                     else uSSRCReuseCount[chnum2]++;

                     if (!nSSRCDuplicationDetection[chnum]) {  /* use the new channel as a sticky flag to limit number of detection messages */

                        char tmpstr[400];
                        sprintf(tmpstr, "=== INFO: session %d channel %d %s session %d channel %d SSRC value 0x%x", hSession2, chnum2, nSSRCDuplicationDetection[chnum2] != chnum+1 ? "duplicating" : "reusing", hSession, chnum, stream_ssrc);  /* check for channel SSRC now being reused */
                        if (term_uFlags[hSession][i] & TERM_ENABLE_DORMANT_SESSION) sprintf(&tmpstr[strlen(tmpstr)], ", flushing dormant channel %d last active %lld msec", chnum, (long long int)((cur_time - last_buffer_time[chnum])/1000));
                        Log_RT(4, tmpstr, "%s \n");

                        nSSRCDuplicationDetection[chnum] = chnum2+1;  /* set SSRC duplication flag (add one to ensure non-zero value) */
                     }
                     #endif
                  }
               ...
               /* modify num_chan for hSession and force a call to DSGetOrderedPackets() to retrieve any of the dormant channel's packets still in its jitter buffer and set a state var indicating the channel is now considered dormant (which will be reset if the channel starts receiving again) */
                  ...
                  if (nDormantChanFlush[hSession][i]) {
                     ...
                     for (int k=0; k<num_chan; k++) if (chan_nums[k] == chnum) { fChanFound = true; break; }
                  ...
                  else { /* printf("\n *** session %d term %d set to dormant \n", hSession, i); */ session_info_thread[hSession].ssrc_state[i] = SSRC_DORMANT; }  /* set this channel's SSRC state to "dormant", prevent further detection */
               }
            }
         }
      }
   ...
   for (i=0; i<MAX_TERMS; i++) {
      ...
      if (nOnHoldChan[hSession][i]) {
         ...
         if (!fChanFound) {
            ...
            for (j=0, numpkts=0; j<num_ch; j++) numpkts = max((int64_t)numpkts, DSGetJitterBufferInfo(ch[j], DS_JITTER_BUFFER_INFO_NUM_PKTS));  /* check if channel's jitter buffer has packets */
...
#define USE_PASTDUE
               ...
               */
               ...
               else if (DSGetJitterBufferInfo(ch[j], DS_JITTER_BUFFER_INFO_CUMULATIVE_TIMESTAMP) < DSGetJitterBufferInfo(ch[j], DS_JITTER_BUFFER_INFO_CUMULATIVE_PULLTIME)) { fFlush = true; chan = ch[j]; break; }  /* analytics mode: check cumulative timestamp vs. cumulative pull time, JHB May 2020 */
            }
...
//  printf("term1.input_buffer_interval = %d, term1.ptime = %d\n", term1.input_buffer_interval, term1.ptime);
   ...
   if (packet_media_thread_info[thread_index].packet_mode) {
      ...
      if (term1.input_buffer_interval < term1.ptime || term2.input_buffer_interval < term2.ptime) {
         ...
         if (fAnalyticsMode) *fAnalyticsMode = true;
      }
   }
   ...
   #if 0
/* input_buffer_interval[] values are set in InitSession() (which is called before InitStream), JHB May 2019 */

   input_buffer_interval[hSession][0] = term1.input_buffer_interval;
   input_buffer_interval[hSession][1] = term2.input_buffer_interval;
   #endif

/* termN uFlags might be altered after session creation, for example in non-library mode (mediaTest cmd line), so we keep them updated, JHB May 2019 */

   term_uFlags[hSession][0] = term1.uFlags;
   term_uFlags[hSession][1] = term2.uFlags;

   session_uFlags[hSession] = DSGetSessionInfo(hSession, DS_SESSION_INFO_HANDLE | DS_SESSION_INFO_UFLAGS, 0, NULL);  /* session flags are less likely to be altered, but we allow for it anyway. zero pulls session flags, 1 or 2 pulls termination info flags */
...
//      session_info_thread[hSession].merge_audio_chunk_size = 4160; /* in samples. Default buffer size is 0.52 sec, maximum can be up to MAX_MERGE_BUFFER_SIZE sec (defined in pktlib.h, figures assume 16-bit samples at 8 kHz output) */
      ...
      session_uFlags[hSession] = DSGetSessionInfo(hSession, DS_SESSION_INFO_HANDLE | DS_SESSION_INFO_UFLAGS, 0, NULL);  /* zero pulls session flags, 1 or 2 pulls termination info flags */
      ...
      for (j=0; j<MAX_TERMS; j++) {
         ...
         int chnum = DSGetSessionInfo(hSession, DS_SESSION_INFO_HANDLE | DS_SESSION_INFO_CHNUM, j+1, 0);
         nSSRCDuplicationDetection[chnum] = 0;
...
/* first two indexes are parent channels */
   ...
   /* build channel list, including child channels belonging to this termN, if any. JHB Nov2019 */
      ...
      for (j=0; j<num_ch; j++) {
         ...
         uNumDamagedFrames[ch[j]] = 0;
         uNumBandwidthEfficientFrames[ch[j]] = 0;
         uNumOctetAlignFrames[ch[j]] = 0;
         uNumCompactFrames[ch[j]] = 0;
         ...
         uSSRCDuplicationCount[ch[j]] = 0;  /* Jul 2025 */
         uSSRCReuseCount[ch[j]] = 0;
      }
   }
...
#ifdef USE_CHANNEL_PKT_STATS
int ManageSessions(HSESSION* hSessions, PKT_COUNTERS pkt_counters[], PKT_STATS_HISTORY input_pkts[], PKT_STATS_HISTORY pulled_pkts[], bool* fAllSessionsDataAvailable, int thread_index, uint64_t cur_time) {
#else
...
int i, j, numSessions = 0, numSessionsFound;
HSESSION hSession;
...
get_num_sessions:
   ...
   memset(hSessions, -1, sizeof(HSESSION)*MAX_SESSIONS);  /* in case numSessions is zero, and the loop doesn't run. JHB Jan 2019 */
   ...
   if (numSessions) for (i=0; i<MAX_SESSIONS; i++) {  /* search for active session handles. Note that pktlib does not re-use deleted session indexes until it wraps around in sessions[] management struct */ 
      ...
      */
         ...
         if (delete_status & DS_SESSION_DELETE_PENDING) {
            ...
            /* DSPostProcessStreamGroup() handles anything that is not real-time and might cause delays in stream group live output, such as N-channel wav file handling. We call it after session flush but before session delete, JHB Jan 2020 */
               ...
               if ((lib_dbg_cfg.uPktStatsLogging & DS_ENABLE_PACKET_TIME_STATS) || (lib_dbg_cfg.uPktStatsLogging & DS_ENABLE_PACKET_LOSS_STATS)) {
                  ...
                  DSLogRunTimeStats(hSession, DS_LOG_RUNTIME_STATS_ORGANIZE_BY_STREAM_GROUP | DS_LOG_RUNTIME_STATS_SUPPRESS_ERROR_MSG | DS_LOG_RUNTIME_STATS_CONSOLE | DS_LOG_RUNTIME_STATS_EVENTLOG);
               }
      ...
      /* update flags likely to change while session is running */
      ...
      /* update session flags and termination flags that might change while session is active, JHB Sep 2024 */
  
         session_uFlags[hSession] = DSGetSessionInfo(hSession, DS_SESSION_INFO_HANDLE | DS_SESSION_INFO_UFLAGS | DS_SESSION_INFO_SUPPRESS_ERROR_MSG, 0, NULL);  /* zero pulls session flags, 1 or 2 pulls termination flags */
         for (j=0; j<MAX_TERMS; j++) term_uFlags[hSession][j] = DSGetSessionInfo(hSession, DS_SESSION_INFO_HANDLE | DS_SESSION_INFO_UFLAGS | DS_SESSION_INFO_SUPPRESS_ERROR_MSG, j+1, NULL);

         fTermFlagsUpdate[hSession] = false;  /* set in DSSessionInfo(DS_SESSION_INFO_TERM_FLAGS), cleared here, JHB Jul 2025 */
   ...
   if (!fEarlyExit && numSessionsFound != numSessions) {
      ...
      if (abs(numSessions - numSessionsFound) > 0) {  /* pktlib sem locks are not used in DSGetSessionInfo() so it's possible we're off by one. Allow one retry. We do not want to be using semaphores when searching 512+ sessions */
         ...
         if (nRetry < nManageSessionRetriesAllowed) {  /* nManageSessionRetriesAllowed is a global with default value of 1, so it can be changed if necessary, JHB Jan 2020 */
            nRetry++;
...
#if 0
   return numSessions;
#else  /* changed to be compatible with "early exit" handling (see fEarlyExit above), JHB Jan 2020 */
...
int i;
   ...
   /* if dormant SSRC flushing is disabled, then combine SSRC and channel number to uniquely define streams, JHB Jul 2024 */

      if (hSession >= 0) uFlags_log |= !((unsigned int)DSGetSessionInfo(hSession, DS_SESSION_INFO_HANDLE | DS_SESSION_INFO_UFLAGS, 1, NULL) & TERM_ENABLE_DORMANT_SESSION) ? DS_PKTSTATS_MATCH_CHNUM : 0;  /* Term1 flags for specific hSession */
         ...
         if (!(term_uFlags[i][0] & TERM_ENABLE_DORMANT_SESSION) || !(term_uFlags[i][1] & TERM_ENABLE_DORMANT_SESSION)) {  /* unless dormant session functionality is enabled we set the combined ssrc + chnum logging flag, JHB Jul 2024 */
...
TERMINATION_INFO  termInfo, termInfo_link;
uint64_t          cur_time;
uint64_t          last_time[MAX_STREAMS] = { 0 };
   ...
   /* incoming packets -- get next packet from all input pcap files specified in cmd line for this thread (-i cmd line option) */
      ...
      for (i = threadid; i < nInFiles; i += nThreads_gbl) {
         ...
         if (cur_time - last_time[i] >= RealTimeInterval[i]*1000) {  /* has interval elapsed ?  (comparison is in usec) */
            ...
            if (!(packet_length = DSReadPcap(fp_in[i], 0, pkt_buffer, NULL, link_layer_length[i], NULL, NULL, NULL, 0, NULL))) continue;
...
struct udphdr *udp_hdr;
   ...
   if (version == IPv4)
   {
      ...
      ip_hdr->saddr = session_data->term1.remote_ip.uIpv4;  /* source IP addr */
      ip_hdr->daddr = session_data->term1.local_ip.uIpv4;  /* dest IP addr */
      #else
      memcpy(&pkt_buffer[IPV4_ADDR_OFS], &session_data->term1.remote_ip.ipv4, IPV4_ADDR_LEN);  /* source IP addr */
      memcpy(&pkt_buffer[IPV4_ADDR_OFS+IPV4_ADDR_LEN], &session_data->term1.local_ip.ipv4, IPV4_ADDR_LEN);  /* dest IP addr */
      #endif
   }
   else if (version == IPv6)
   {
      ...
      memcpy(ipv6_hdr->saddr.s6_addr, session_data->term1.remote_ip.ipv6, IPV6_ADDR_LEN);  /* source IP addr */
      memcpy(ipv6_hdr->daddr.s6_addr, session_data->term1.local_ip.ipv6, IPV6_ADDR_LEN);  /* dest IP addr */
      #else
      memcpy(&pkt_buffer[IPV6_ADDR_OFS], &session_data->term1.remote_ip.ipv6, IPV6_ADDR_LEN);  /* source IP addr */
      memcpy(&pkt_buffer[IPV6_ADDR_OFS+IPV6_ADDR_LEN], &session_data->term1.local_ip.ipv6, IPV6_ADDR_LEN);  /* dest IP addr */
      #endif
   }
   else
   {
...
void RecordPacketTimeStats(int chnum, uint8_t* pkt, int pkt_len, unsigned int pkt_info, uint32_t pkt_count, unsigned int uFlags) {

uint64_t packet_time;
uint32_t rtp_timestamp, Fs;
int idx;
   ...
   if (uFlags == PACKET_TIME_STATS_INPUT) {
      ...
      if (last_rtp_timestamp[chnum]) {  /* note that input RTP timestamps may be ooo, so we use int calculations here and allow negative values. They will still add up correctly, JHB Jan 2020 */
         ...
         if (Fs) packet_rtp_time[chnum] += 1000*((int64_t)rtp_timestamp - (int64_t)last_rtp_timestamp[chnum])/Fs;  /* save in msec (and avoid divide-by-zero, JHB May 2019) */
      }
      ...
      if ((idx = DSGetStreamGroupInfo(chnum, DS_STREAMGROUP_INFO_HANDLE_CHNUM, NULL, NULL, NULL)) >= 0) pkt_count = ++pkt_count_group[idx];  /* overwrite input pkt_count */
      ...
      if (last_packet_in_time[chnum]) {
         ...
         if ((pkt_info == DS_PKT_PYLD_CONTENT_MEDIA || pkt_info == DS_PKT_PYLD_CONTENT_MEDIA_REUSE) && (prev_pyld_content[chnum] == DS_PKT_PYLD_CONTENT_MEDIA || prev_pyld_content[chnum] == DS_PKT_PYLD_CONTENT_MEDIA_REUSE)) {  /* bug-fix: also check for DS_PKT_PYLD_CONTENT_MEDIA_REUSE, JHB Nov 2024 */
         ...
         else if (pkt_info == DS_PKT_PYLD_CONTENT_SID && prev_pyld_content[chnum] == DS_PKT_PYLD_CONTENT_SID) {
         ...
         prev_pyld_content[chnum] = pkt_info;
      }
      ...
      if (last_rtp_timestamp_pull[chnum]) {
         ...
         Fs = DSGetSessionInfo(chnum, DS_SESSION_INFO_CHNUM | DS_SESSION_INFO_INPUT_SAMPLE_RATE, 0, NULL);
         if (!Fs) Fs = DSGetSessionInfo(chnum, DS_SESSION_INFO_CHNUM | DS_SESSION_INFO_SAMPLE_RATE, 0, NULL);  /* use decode sample rate if no input sample rate was assigned */
         if (Fs) packet_rtp_time_pull[chnum] += 1000*((int64_t)rtp_timestamp - (int64_t)last_rtp_timestamp_pull[chnum])/Fs;  /* save in msec (and avoid divide-by-zero, JHB May 2019) */
      }
...
#if 0  /* referenced by CleanSession() so actually declared with other statics. But we mention it here for readability */
static uint8_t session_run_time_stats[NCORECHAN] = { 0 };  /* keep track of session stats logging, avoid repeating each time DSLogRunTimeStats() is called */
#endif
...
int  idx = -1, nc = 0, nContributors = 0, ch_list[MAX_GROUP_CONTRIBUTORS], num_sessions = 0, num_ch_stats = 0, nCombinedCount = 0;
bool fNext, fOrganizeByStreamGroup[MAX_GROUP_CONTRIBUTORS] = { false }, fShowOwnerOnce = false;  /* make fOrganizeByStreamGroup an array to handle mix of stream group member and non-member, JHB Sep 2024 */
HSESSION hSessionGroupOwner = -1, hSession_prev = -1;

#define MAX_SESSIONS_PER_STATS_DISPLAY 8
#define ISL '|'  /* item separator literal, we were using ':' but changed to '|' because prefixing with : can cause confusion in time stats specified sec:msec or min:sec, JHB Mar 2025 */
...
char iptstr[MAX_STATS_STRLEN] = "", jbptstr[MAX_STATS_STRLEN] = "", jbrpstr[MAX_STATS_STRLEN] = "", jbzpstr[MAX_STATS_STRLEN] = "", mxooostr[MAX_STATS_STRLEN] = "", sidrstr[MAX_STATS_STRLEN] = "", tsastr[MAX_STATS_STRLEN] = "", plflstr[MAX_STATS_STRLEN] = "",
   ...
   if (uFlags & DS_LOG_RUNTIME_STATS_ORGANIZE_BY_STREAM_GROUP) {
      ...
      if (hSessionGroupOwner < 0) {
         ...
         Log_RT(3, "WARNING: DSLogRunTimeStats() says DSGeSessionInfo() returns group owner session %d using hSession %d \n", hSessionGroupOwner, hSession);
         return -1;
      }
      ...
      for (i=0; i<nContributors; i++) fOrganizeByStreamGroup[i] = true;  /* indicate channel is a stream group member, JHB Sep 2024 */
   }
   else {
...
organize_by_session:
   ...
   /* if session is marked for deletion we assume we're here from ManageSessions() */
      ...
      bool fDeletePending = (DSGetSessionInfo(hSession, DS_SESSION_INFO_HANDLE | DS_SESSION_INFO_DELETE_STATUS, 0, NULL) & DS_SESSION_DELETE_PENDING) != 0;  /* note: accessing session delete status is reserved ... don't use it unless you know what you're doing, JHB Jan 2020. Moved this inside the do-loop to fix bug with stats for all sessions not being fully displayed in some cases. Test with cmd line mediaMin -cx86 -i../pcaps/AMRWB.pcap -i../pcaps/pcmutest.pcap -L -d0x08000c11 -r20 -g /tmp/shared, JHB Dec 2024 */

      num_ch = 0;

      if (fOrganizeByStreamGroup[nc]) hSession = DSGetSessionInfo(ch_list[nc], DS_SESSION_INFO_CHNUM | DS_SESSION_INFO_SESSION, 0, NULL);
      ...
      for (j=0; j<MAX_TERMS; j++) {
         num_dyn_ch = DSGetSessionInfo(hSession, DS_SESSION_INFO_HANDLE | DS_SESSION_INFO_DYNAMIC_CHANNELS, j+1, (void*)&ch[num_ch]);  /* need typecast here, otherwise compiler messes up. Not sure why, JHB Feb 2019 */
      ...
      for (i=0; i<num_ch; i++) {
         ...
         if (fDeletePending) {
            if (session_run_time_stats[c] & 1) continue;  /* run-time stats for this channel have already been logged prior to deletion, don't log them twice, JHB Jan 2020 */
            else session_run_time_stats[c] |= 1;
         }

      /* parse the channel list and build stats display strings. Notes:

         -we show stats for any stream with at least one packet. Note this will exclude streams that never had a matching packet, or didn't start yet, for example mediaMin was terminated early

         -DSCreateSession() always creates 2 channels (2 terminations), so both always exist, which is why DSGetSessionInfo() calls with a valid session handle and specifying either channel or term always succeed. But they may not be active (no media), another reason to use the "at least one packet" filter
      */
         ...
         if (DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_INPUT_PKT_COUNT | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING)) {
            ...
            if (lib_dbg_cfg.uPktStatsLogging & DS_ENABLE_PACKET_TIME_STATS) {
               ...
               add_stats_str(medstr, MAX_STATS_STRLEN, " %d%c%2.2f", c, ISL, timeScale*packet_media_delta[c]/media_stats_pkt_count[c]/1000);
               add_stats_str(sidstr, MAX_STATS_STRLEN, " %d%c%2.2f", c, ISL, timeScale*packet_sid_delta[c]/sid_stats_pkt_count[c]/1000);
               add_stats_str(medxstr, MAX_STATS_STRLEN, " %d%c%2.2f/%d", c, ISL, timeScale*packet_max_media_delta[c]/1000, max_media_delta_packet[c]);
               add_stats_str(sidxstr, MAX_STATS_STRLEN, " %d%c%2.2f/%d", c, ISL, timeScale*packet_max_sid_delta[c]/1000, max_sid_delta_packet[c]);
               add_stats_str(maxdstr, MAX_STATS_STRLEN, " %d%c%2.2f/%d", c, ISL, timeScale*packet_max_delta[c]/1000, max_delta_packet[c]);
               add_stats_str(iptstr, MAX_STATS_STRLEN, " %d%c%2.2f/%2.2f", c, ISL, timeScale*packet_in_time[c]/1000000L, 1.0*packet_rtp_time[c]/1000);
               add_stats_str(jbptstr, MAX_STATS_STRLEN, " %d%c%2.2f/%2.2f", c, ISL, timeScale*packet_in_time_pull[c]/1000000L, 1.0*packet_rtp_time_pull[c]/1000);
            }
            ...
            DSGetCodecInfo(termInfo.codec_type, DS_CODEC_INFO_NAME, 0, 0, codec_name);

            sprintf(codecinfostr, "%s-%d", codec_name, termInfo.bitrate);  /* show initial bitrate */

            if (DSGetCodecInfo(termInfo.codec_type, DS_CODEC_INFO_BITRATE_TO_INDEX, termInfo.bitrate, 0, NULL) >= 0) {  /* sanity check - make sure the initial bitrate is in DSGetCodecInfo() index tables */
               ...
               for (j=0; j<(int)sizeof(uint32_t)*CHAR_BIT; j++) {  /* show any mid-stream switching bitrates detected */
                  ...
                  if ((pkt_bitrate_list & (1L << j)) && (bitrate = DSGetCodecInfo(termInfo.codec_type, DS_CODEC_INFO_INDEX_TO_BITRATE, j, 0, NULL)) >= 0 && bitrate != (int)termInfo.bitrate) {
            ...
            if (fShow_audio_classification) {  /* true if --show_aud_clas is on the cmd line; see cmd_line_interface.c, JHB Feb 2024 */
               ...
               unsigned int pkt_classification_list = DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_PKT_CLASSIFICATION_LIST | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING);

               for (j=1; j<=(FRAMETYPE_ITEM_MASK+1); j <<= 1) {  /* iterate through FRAMETYPE_XXX audio classification enums */

                  int audio_classification;

                  if ((pkt_classification_list & j) && (audio_classification = DSGetCodecInfo(termInfo.codec_type, DS_CODEC_INFO_LIST_TO_CLASSIFICATION, j, 0, NULL)) >= 0) sprintf(&codecinfostr[strlen(codecinfostr)], "-%s", (char*)&audio_classification);  /* DSGetCodecInfo() with DS_CODEC_INFO_LIST_TO_CLASSIFICATION flag returns 0-3 char string, add to codecinfo string */
               }
            }

            if (hSession != hSession_prev) {

               add_stats_str(sessstr, MAX_STATS_STRLEN2, " %d%s%c%d%c%s", hSession, hSession == hSessionGroupOwner && !fShowOwnerOnce ? "(grp owner)" : "", ISL, c, ISL, codecinfostr);
            }
            else add_stats_str(sessstr, MAX_STATS_STRLEN2, ",%d%c%s", c, ISL, codecinfostr);  /* add channel to previous session string, showing it as either additional termination or dynamic (child) channel */

            if (fOrganizeByStreamGroup[nc]) {

               if (DSGetSessionInfo(c, DS_SESSION_INFO_CHNUM | DS_SESSION_INFO_CHNUM_PARENT, 0, NULL) == c) {  /* add overrun stats only for parent channels (as explained in streamlib.h, child channels contribute to their parent group member stream), JHB Apr 2020 */

                  #ifdef __LIBRARYMODE__
                  add_stats_str(ovrnstr, MAX_STATS_STRLEN, " %d%c%u", c, ISL, uFramesDropped[c]);

                  HCODEC hCodec;
                  int framesize;

                  if ((hCodec = DSGetSessionInfo(c, DS_SESSION_INFO_CHNUM | DS_SESSION_INFO_CODEC, 1, NULL)) > 0 && (framesize = DSGetCodecInfo(hCodec, DS_CODEC_INFO_HANDLE | DS_CODEC_INFO_RAW_FRAMESIZE, 0, 0, NULL)) > 0) add_stats_str(mxovrnstr, MAX_STATS_STRLEN, " %d%c%2.2f", c, ISL, 100.0*nMaxStreamDataAvailable[c]/framesize/DSGetStreamGroupContributorMaxFrameCapacity(c));
                  #endif
               }
            ...
            if ((uTimestampMatchMode & TIMESTAMP_MATCH_MODE_ENABLE) && (uTimestampMatchMode & TIMESTAMP_MATCH_ENABLE_STREAM_SYNC)) {

               #ifdef __LIBRARYMODE__
               add_stats_str(synccompstr, MAX_STATS_STRLEN, " %d%c%u", c, ISL, uStreamSyncCompensations[c]);
               #endif
            }
         ...
         /* SSRCs */
            ...
            add_stats_str(ssrcstr, MAX_STATS_STRLEN2, " %d%c0x%x%c%d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_SSRC | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING), ISL, uSSRCDuplicationCount[c], ISL, uSSRCReuseCount[c]);

         /* input packet type counts */

            add_stats_str(npktstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_INPUT_PKT_COUNT | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(spktstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_INPUT_SID_COUNT | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(dtmfstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_NUM_DTMF_PKTS | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));

            add_stats_str(dupstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_NUM_7198_DUPLICATE_PKTS | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));

            add_stats_str(brststr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, packet_in_bursts[c]);

         /* loss */

            add_stats_str(pktlstr, MAX_STATS_STRLEN, " %d%c%2.3f", c, ISL, 100.0*DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_MISSING_SEQ_NUM | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING)/max(DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_INPUT_PKT_COUNT | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING), (int64_t)1));

            add_stats_str(plflstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, pkt_loss_flush[c]);
            ...
            if (lib_dbg_cfg.uPktStatsLogging & DS_ENABLE_PACKET_LOSS_STATS) {  /* run-time packet loss stats can be disabled in pktlib in case they cause any issue, JHB Mar 2020 */
               ...
               add_stats_str(iooostr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_NUM_INPUT_OOO | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
               add_stats_str(mxooostr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_MAX_INPUT_OOO | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
               add_stats_str(calcstr, MAX_STATS_STRLEN, " %d%c%2.2f", c, ISL, 1.0*DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_STATS_CALC_PER_PKT | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING)/max(DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_INPUT_PKT_COUNT | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING), (int64_t)1));
            }

         /* gaps, JHB Nov 2023 */

            add_stats_str(gapstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, uTimestampGapCount[c]);
            add_stats_str(onholdstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, uGapWindowActiveCount[c]);

            add_stats_str(dmgfrmstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, uNumDamagedFrames[c]);
            add_stats_str(bwefrmstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, uNumBandwidthEfficientFrames[c]);
            add_stats_str(octfrmstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, uNumOctetAlignFrames[c]);
            add_stats_str(cmpfrmstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, uNumCompactFrames[c]);
         ...
         /* misc */
            ...
            add_stats_str(jbrpstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_NUM_PKTS | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));  /* residual num pkts */
            add_stats_str(jbtssetbacksstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_NUM_TIMESTAMP_SETBACKS | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));  /* timestamp setbacks, JHB Feb 2025 */
            add_stats_str(jbzpstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, num_jb_zero_pulls[c]);

         /* repair stats */

            add_stats_str(sidrstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_SID_REPAIR | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(tsastr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_SID_TIMESTAMP_ALIGN | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(tsamstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_MEDIA_TIMESTAMP_ALIGN | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(sidistr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_SID_REPAIR_INSTANCE | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));

         /* jitter buffer stats */

            add_stats_str(noutpkts, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_OUTPUT_PKT_COUNT | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(mxnpktstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_MAX_NUM_PKTS | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(jbdropstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_NUM_OUTPUT_DROP_PKTS | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(jbdupstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_NUM_OUTPUT_DUPLICATE_PKTS | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(jboooostr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_NUM_OUTPUT_OOO | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(jbmxooostr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_MAX_OUTPUT_OOO | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(purgstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_NUM_PURGES | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(jbundrstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_UNDERRUN_RESYNC_COUNT | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(jboverstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_OVERRUN_RESYNC_COUNT | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(jbtgapstr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_TIMESTAMP_GAP_RESYNC_COUNT | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(jbhldadj, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_NUM_HOLDOFF_ADJUSTS | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(jbhlddel, MAX_STATS_STRLEN, " %d%c%d", c, ISL, DSGetJitterBufferInfo(c, DS_JITTER_BUFFER_INFO_NUM_HOLDOFF_DELIVERIES | DS_JITTER_BUFFER_INFO_ALLOW_DELETE_PENDING));
            add_stats_str(pobrststr, MAX_STATS_STRLEN, " %d%c%d", c, ISL, packet_out_bursts[c]);
         }
      ...
      if (fOrganizeByStreamGroup[nc] && ++nc < nContributors) fNext = true;

      if (!fNext && nCombinedCount < MAX_SESSIONS_PER_STATS_DISPLAY) {  /* modify stats display behavior to combine sessions in format similar to stream groups, instead of displaying each session separately. We limit the number sessions per stats display due to string lengths, JHB Aug 2023 */
         ...
         if (hSession+1 < MAX_SESSIONS) {  /* look for next active session */ 
            ...
            if (hSession >= 0 && isSessionAssignedToThread(hSession, thread_index)) fNext = true;
         }
      }

      if (fNext) nCombinedCount++;
   ...
   */
      ...
      if (fOrganizeByStreamGroup[0]) add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "stream group \"%s\", grp %d, p/m thread %d, num packets %d \n", szGroupId, idx, thread_index, pkt_count_group[idx]);
      add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "%session%s (hSession%cch%ccodec-bitrate[,ch...])%s\n", fOrganizeByStreamGroup[0] ? "  S" : "s", num_sessions > 1 ? "s" : "", ISL, ISL, sessstr);

      add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "  SSRC%s (ch%cssrc%cduplications%creuses)%s\n", num_ch_stats > 1 ? "s" : "", ISL, ISL, ISL, ssrcstr);

      if (fOrganizeByStreamGroup[0]) {
         add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "  Overrun (ch%cframes dropped)%s, (ch%cmax)%s\n", ISL, ovrnstr, ISL, mxovrnstr);
         add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "  Underrun (grp%cmissed intervals/FLCs/holdoffs)%s\n", ISL, undrstr);
      }

      if ((uTimestampMatchMode & TIMESTAMP_MATCH_MODE_ENABLE) && (uTimestampMatchMode & TIMESTAMP_MATCH_ENABLE_STREAM_SYNC)) {

         add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "  TSM stream sync (ch%clag compensation)%s\n", ISL, synccompstr);
      }

      add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "  Pkt flush (ch%cnum) loss%s, pastdue%s, level%s\n", ISL,plflstr, pdflstr, lvflstr);

      add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "  Packet Stats\n");
      add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "    Input (ch%cpkts)%s, SID%s, DTMF%s, RFC7198 duplicate%s, burst%s\n", ISL, npktstr, spktstr, dtmfstr, dupstr, brststr);

      if (lib_dbg_cfg.uPktStatsLogging & DS_ENABLE_PACKET_LOSS_STATS) {
         add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "    Loss (ch%c%)%s, missing seq (ch%cnum)%s, max consec missing seq%s\n", ISL, pktlstr, ISL, missstr, consstr);
         add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "    Input Ooo (ch%cpkts)%s, max%s\n", ISL, iooostr, mxooostr);
      }

      add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "    Timestamp gaps (ch%cpkts)%s, On-hold%s\n", ISL, gapstr, onholdstr);

      if (lib_dbg_cfg.uPktStatsLogging & DS_ENABLE_PACKET_TIME_STATS) {

         add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "    Delta avg (ch%cmsec) media%s, SID%s\n", ISL, medstr, sidstr);
         add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "    Delta max (ch%cmsec/pkt) media%s, SID%s, overall%s\n", ISL, medxstr, sidxstr, maxdstr);
         add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "    Cumulative input times         (sec) (ch%cinp/rtp)%s\n", ISL, iptstr);
         add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "    Cumulative jitter buffer times (sec) (ch%cout/rtp)%s\n", ISL, jbptstr);
      }

      add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "  Packet Repair\n");
      add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "    SID repair (ch%cnum) instance%s, total%s\n", ISL, sidistr, sidrstr);
      add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "    Timestamp repair (ch%cnum) SID%s, media%s\n", ISL, tsastr, tsamstr);


      add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "  Jitter Buffer\n");
      add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "    Output (ch%cpkts)%s, max depth%s, residual%s, bursts%s\n", ISL, noutpkts, mxnpktstr, jbrpstr, pobrststr);
      add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "    Output Ooo (ch%cpkts)%s, max%s, Drops%s, Duplicates%s\n", ISL, jboooostr, jbmxooostr, jbdropstr, jbdupstr);
      add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "    Resyncs (ch%cnum) underrun%s, overrun%s, timestamp gap%s, Purges (ch%cnum)%s\n", ISL, jbundrstr, jboverstr, jbtgapstr, ISL, purgstr);

      char allocscurstr[50], allocsmaxstr[50];  /* not per-channel */
      ...
      add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "    Holdoffs (ch%cnum) adj%s, dlvr%s, Timestamp setbacks (ch%cnum)%s, Zero pulls%s, Allocs (cur/max)%s/%s\n", ISL, jbhldadj, jbhlddel, ISL, jbtssetbacksstr, jbzpstr, allocscurstr, allocsmaxstr);
   ...
   /* Alternatively we can use DS_LOG_LEVEL_SUBSITUTE_WEC flag to avoid false-positive keyword search. Given warnings, errors, critical this will produce w|arnings, e|rrors, c|ritical. See comments in shared_include/config.h, JHB Jan 2021 */
      add_stats_str(pkt_stats_str, MAX_PKT_STATS_STRLEN, "  Event log warnings, errors, critical èrrors %u, %u, %u\n", __sync_fetch_and_add(&event_log_warnings, 0), __sync_fetch_and_add(&event_log_errors, 0), __sync_fetch_and_add(&event_log_critical_errors, 0));
      #endif

      if ((uFlags & (DS_LOG_RUNTIME_STATS_EVENTLOG | DS_LOG_RUNTIME_STATS_CONSOLE))) {  /* output if either flag set, JHB Mar 2021 */
         ...
         #else  /* DS_LOG_LEVEL_OUTPUT__XXX definitions and usage have been simplified in config.h, JHB Nov 2024 */
         if (uFlags & DS_LOG_RUNTIME_STATS_EVENTLOG) uFlags_LogRT |= DS_LOG_LEVEL_OUTPUT_FILE;  /* set DS_LOG_LEVEL_OUTPUT_XX flags for screen and/or file, if applicable. JHB Mar 2021 */
         if (uFlags & DS_LOG_RUNTIME_STATS_CONSOLE) uFlags_LogRT |= DS_LOG_LEVEL_OUTPUT_CONSOLE;
         #endif
...
bool fNextLine = false;
   ...
   if (!fOnce && isMasterThread(thread_index)) {  /* only possible reason this could be useful is if for some reason the app didn't call DSConfigPktlib(), which would be bad app behavior */
   ...
   #if 0
   if ((!(level & PRN_SAME_LINE) /*|| uLineCursorPos > strlen(prnstr)+2*/) && __sync_val_compare_and_swap(&isCursorMidLine, 1, 0)) fNextLine = true;  /* fNextLine reflects leading \n decision. For description of the semaphore logic, see comments in event_logging.cpp in diaglib subfolder. Note that user_io.cpp also uses the semaphore (they all have basically same logic) */
   else if (prnstr[slen-1] != '\n' && prnstr[slen-1] != '\r') __sync_val_compare_and_swap(&isCursorMidLine, 0, 1);

   #else  /* separate leading \n decision from isCursorMidLine test and set, JHB Apr 2025 */

   fNextLine = (!(level & PRN_SAME_LINE) && isCursorMidLine) || isLinePreserve;

   #endif

   uLineCursorPos = (slen && (prnstr[slen-1] == '\n' || prnstr[slen-1] == '\r')) ? 0 : (prnstr[0] == '\r' ? 0 : uLineCursorPos) + slen;  /* update line cursor position. Update to include checks for slen > 0 and \r, fix calculation as pos + len instead of just len, JHB Apr 2025 */

   #if 0
   if ((slen && (prnstr[slen-1] == '\n' || prnstr[slen-1] == '\r')) || prnstr[0] == '\r')  __sync_val_compare_and_swap(&isCursorMidLine, 1, 0);  /* if one set to zero */
   ...
   console_out(lib_dbg_cfg.uPrintfControl, 4, fNextLine, prnstr);  /* write to console: type of output, log level 4 (errors and warnings are 0..3), no next line (console_out() is in diaglib_util.cpp), JHB Jul 2025 */
...
void set_session_last_push_time(HSESSION hSession) {
   ...
   __sync_lock_test_and_set(&last_push_time[hSession], last_cur_time[get_session_thread_index(hSession)]);  /* get_session_thread_index() is inline in pktlib.h, JHB Jul 2025 */
...
char tmpstr[8000] = "";
   ...
   sprintf(&tmpstr[strlen(tmpstr)], "buffer pkts = %2.2f, decode pkts = %2.2f, encode pkts = %2.2f, stream group contributions = %2.2f \n", 1.0*buf_pkt_sum/max(num_buf_counted, (uint64_t)1), 1.0*enc_pkt_sum/max(num_enc_counted, (uint64_t)1), 1.0*dec_pkt_sum/max(num_dec_counted, (uint64_t)1), 1.0*group_contrib_sum/max(num_stream_group_counted, (uint64_t)1));
...
/* gather group info about sessions in this thread */
   ...
   if (hSessions_t) {  /* NULL if called from ThreadAbort() */
      ...
      if (numSessions > 0) {
         ...
         for (i=0; i<numSessions; i++) {
            ...
            uint32_t pkt_cnt =  min(pkt_count[hSessions_t[i]], (uint32_t)DELTA_SUM_LENGTH);
   ...
   */
   ...
   sprintf(&tmpstr[strlen(tmpstr)], "event log info: cumulative wàrnings = %u, èrrors = %u, crìtical èrrors = %u \n", __sync_fetch_and_add(&event_log_warnings, 0), __sync_fetch_and_add(&event_log_errors, 0), __sync_fetch_and_add(&event_log_critical_errors, 0));
   ...
   if (uFlags & DS_DISPLAY_THREAD_DEBUG_INFO_SCREEN_OUTPUT) sig_printf(tmpstr, PRN_NO_LEVEL, thread_index);  /* use buffered I/O, JHB Jan 2020 */
