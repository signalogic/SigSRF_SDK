/*
 ...
 Revision History
   ...
   Modified Dec 2021 JHB, add experimental handling of .ber files on command line. Note the file input type definition IO_TYPE_BER added in pktlib.h
   ...
   Modified Nov 2023 JHB, implement IO_TYPE_RTP to handle .rtpdump (and .rtp) format inputs. Heavy lifting is done in pktlib DSOpenPcap() and DSReadPcap(). See the modified pcap_hdr_t struct in pktlib.h
   ...
   Modified Sep 2025 JHB, improve error handling in CreateDynamicSession(), replace thread_info[].init_err with .uErrorCondition to improve differentiation of initialization and run-time errors
   Modified Sep 2025 JHB, simplify some code with getIOType() and isInputXxx() macro (pktlib.h)
*/
...
#endif
static char version_str[] = "v3.9.2";
...
char          szSessionName[MAX_STREAMS_THREAD][384] = {{ "" }};  /* set in LoggingSetup() which should always be called. Referenced in session_app.cpp */
...
/* mediaMin application entry point. Program and multithreading notes:
   ...
   -mediaMin and mediaTest have the same command line format. Key differences include (i) mediaMin recognizes many more -dN options for operating modes and flags, (ii) handling of I/O (different sets of input file types), and (iii) mediaMin ignores -Ex and -tN entry (used only by mediaTest. Command line documentation is at https://github.com/signalogic/SigSRF_SDK/blob/master/mediaTest_readme.md#user-content-commandlinequick-reference_main)
*/
...
HSESSION       hSessions[MAX_SESSIONS_THREAD] = { 0 };  /* MAX_SESSIONS_THREAD is max number of concurrent sessions per thread. For capacity tests that use the -RN (repeat N) command line option, this assumes sessions will complete (i.e. either input ends or SIP BYE message occurs) before the next repeat */
...
uint8_t uExitErrorCond = 0;  /* set true on error conditions */
char tmpstr[MAX_APP_STR_LEN];  /* large temporary string used for various purposes. MAX_APP_STR_LEN is defined in diaglib.h */
...
/* check for any setup errors */
   ...
   if (thread_info[thread_index].uErrorCondition && !fThreadSync2) goto cleanup;  /* setup error occurred. If this is first time through we clean up and exit, if not, then the user can see which threads / repeat runs have the error and quit as needed */
...
/* all initialization complete, begin continuous packet push-pull loop */
   ...
   /* read from packet input flows, push to packet/media threads */
      ...
      if (!(Mode & USE_PACKET_ARRIVAL_TIMES)) if (PushPackets(pkt_in_buf, hSessions, session_data, thread_info[thread_index].nSessionsCreated, cur_time, thread_index) < 0) if (thread_info[thread_index].uErrorCondition) break;
...
/* either 1) exit on quit, stop, or error condition, or 2) repeat depending on test condition (for the latter, look for "nRepeatsRemaining") */

   #if 0
   uExitErrorCond = thread_info[thread_index].uErrorCondition && (num_app_threads == 1 || thread_index > 0 || !fThreadSync2);
   #else
   uExitErrorCond = thread_info[thread_index].uErrorCondition && (num_app_threads == 1 || thread_index > 0 || !fThreadSync2) ? thread_info[thread_index].uErrorCondition : 0;
   #endif

   bool fExit = fQuit || fStop || uExitErrorCond;
...
/* display and log summary stats */
   ...
   if (uExitErrorCond != 1 && !fCreateDeleteTest && !fCapacityTest) DisplayLogSummaryStats(tmpstr, sizeof(tmpstr), cur_time, thread_index);  /* don't display summary stats 1) for initialization error conditions, or 2) in stress test modes, in which the stats string can be very long and obscure the event log and/or clog buffered I/O to remote terminals */
...
err_msg:
      ...
      else Log_RT(3, "mediaMin WARNING: %s \n", errstr);

      return -1;  /* return warning or error condition */
   }
...
*/
   ...
   if (codec_type <= 0) {  /* values < 0 indicate error condition, 0 indicates a codec was not detected */
      ...
      else if (codec_type < 0) { Log_RT(3, "mediaMin WARNING: detect_codec_type_and_bitrate() returns error condition \n"); return -1; }  /* error condition */
...
// if (!(Mode & COMBINE_INPUT_SPECS) && nStream > 0) printf("\n ============= group_id = %s, prev group id = %s \n", group_id, thread_info[thread_index].szGroupName[nStream-1]);
      ...
      if (!fCreateDeleteTest && !fCapacityTest) {
         ...
         sprintf(session->szSessionName, "%s%s", szOutputMediaPath, thread_info[thread_index].szGroupName[nStream]);  /* set session->szSessionName for wav outputs. Note we currently don't allow wav outputs during capacity test */
      }
...
/* verify we are not exceeding max number of sessions. To-do: use malloc and realloc for sessions (first row of 2-D array) */

   if (thread_info[thread_index].nSessionsCreated >= MAX_SESSIONS_THREAD) {
   
      Log_RT(2, "mediaMin ERROR: number of sessions exceeds %d \n", MAX_SESSIONS_THREAD);
...
/* create the session */
   ...
   if ((hSession = DSCreateSession(hPlatform, GetSessionFlags(), NULL, session)) < 0) {  /* note - GetSessionFlags() is in session_app.cpp */
      ...
      Log_RT(2, "mediaMin ERROR: CreateDynamicSession() failed to create dynamic session, app thread %d", thread_index); 
      return -2;  /* critical error */
   }
...
/* update stream stats */
   ...
   if (thread_info[thread_index].num_stream_stats < MAX_STREAM_STATS) {  /* MAX_STREAM_STATS is defined in mediamin.h; note that if repeat is enabled num_stream_stats can exceed the limit, in which case (for the time being) we stop recording them */
...
rtp_packet_format_error:
   ...
   #endif
   {
      Log_RT(3, "mediaMin WARNING: CreateDynamicSession() says RTP packet format error, %s, no session creation or codec estimation performed \n", errstr);
   }
   return -1;
}
...
int session_push_cnt[MAX_SESSIONS_THREAD] = { 0 };
...
static uint64_t last_wait_check_time[MAX_STREAMS_THREAD] = { 0 }, wait_pause[MAX_STREAMS_THREAD] = { 0 };  /* only used by master app thread */
...
next_packet:  /* next packet input */
         ...
         int8_t input_type = getIOType(thread_info[tId].link_layer_info[j]);

         if (isInputPcap(input_type) || isInputUDP(input_type)) {  /* handle pcap formats and UDP ports here, BER and other file formats below */
         ...
         /* process non-zero length packets */
            ...
            /* several packet find and filter examples inside #if 0 below, use for stream manipulation, test, and debug if needed */

               #if 0  /* filter packets based on src / dst ports */
               static int nCount = 0;
               if (nCount < 2) {
               ...
               #if 0  /* filter packets based on source IP address */
                __uint128_t src_ip_addr = 0;
               DSGetPacketInfo(-1, DS_BUFFER_PKT_IP_PACKET | DS_PKT_INFO_SRC_ADDR, pkt_buf, -1, &src_ip_addr, NULL, 0);
               if (/*src_ip_addr == 0xc0a88615 || src_ip_addr == 0x0a0e0742 || */ PktInfo.rtp_ssrc == 0xbbf51b9c && (/*PktInfo.rtp_pyld_len == 63 || PktInfo.rtp_pyld_len == 35 || */ PktInfo.rtp_pyld_len == 8)) {  /* 0xc0a88615 -> 192.168.134.21 */
         ...
         else if (isInputBer(input_type)) {  /* experimental BER input handling, JHB Dec 2021 */
            ...
            if (pkt_len > 0 && (Mode & ENABLE_INTERMEDIATE_PCAP)) {  /* format BER data into TCP/IP packet and generate pcap output if specified in cmd line flags, JHB Dec 2021*/
               ...
               PacketActions(ber_data, pkt_buf, (PktInfo.protocol = TCP), &pkt_len, IO_TYPE_BER);
...
protocol_based_processing:
      ...
      /* DER encoded encapsulated stream processing, JHB Mar 2021 */
         ...
         /* DSFindDerStream() finds info at HI2 level, including 1) new DER streams, 2) new destination ports for an existing DER stream */
            ...
            #ifdef BER_DEBUG
            if (isInputBer(input_type)) printf("******** attempting to find intercept ID \n");
            #endif
            ...
            if (DSFindDerStream(pkt_buf, uFlags, &szInterceptPointId[0], der_dst_port_list, thread_info[tId].hFile_ASN_XML[j]) > 0) {
               ...
               #ifdef BER_DEBUG
               if (isInputBer(input_type)) printf("******** found intercept ID \n");
               #endif
...
//  if (pkt_index) printf("updating pkt index = %d, prev index = %d, timestamp = %llu \n", pkt_index, thread_info[tId].EncapsulatedStreamIndex[j], (unsigned long long)pkt_timestamp);
               ...
               if ((Mode & ENABLE_INTERMEDIATE_PCAP) && fFoundEncapsulatedCCPkt) PacketActions(NULL, pkt_buf, PktInfo.protocol, &pkt_len, IO_TYPE_ASN_XML);  /* write out decoded DER stream to intermediate pcap; note this currently only includes decoded UDP packets, JHB Dec 2021 */
            }
      ...
      /* TCP handling. mediaMin SDK version currently handles TCP/IP packets for encapsulated DER streams, SIP info and invites messages, and a few others, but otherwise ignores TCP */
         ...
         if (PktInfo.protocol == TCP) {
            ...
            isPortAllowed(thread_info[tId].dst_port[j], 0, pkt_buf, pkt_len, PktInfo.protocol, j, cur_time, thread_index);  /* allow isPortAllowed() to keep track of some things */
   ...
   //if (thread_info[tId].packet_number[j] >= 204 && thread_info[tId].packet_number[j] <= 215) printf("\n *** after TCP port allow pkt# %u, ret_val = %d \n", thread_info[tId].packet_number[j], ret_val);
         ...
         /* look for SIP invite packets */
            ...
            /* keep track of SIP invites within DER streams */
            ...
            /* FindStream() looks at incoming streams and returns > 0 for a new session, 0 for an existing session. FindStream() is in session_app.c, JHB Nov 2024 */
               ...
               if (nSessionsFound > 0 && !(fInitialStaticSession && nSessionsFound == 1)) {  /* for test modes with an initial static session we ignore the first new session found */
                  ...
                  if (ret_val > 0) {  /* ret_val < 0 is an error condition, error message already logged or displayed */
                     ...
                     if (ret_val == -2) { thread_info[tId].uErrorCondition = 2; return -1; }  /* non I/O related error, message already displayed and logged */
                  }
               }
...
push:
        ...
        if (rtp_seqnum > 37810 && rtp_seqnum < 37825) printf("\n *** pushing rtp seqnum %d \n", rtp_seqnum);
                  ...
                  if (!ret_val) {  /* push queue is full, try waiting and pushing the packet again */
                     ...
                     Log_RT(3, "mediaMin WARNING: error condition returned by DSPushPackets, hSession = %d, pkt_len = %d\n", hSessions[i], pkt_len);
                     continue;
                  }
...
/* read new input data if input cache marked as invalid */
   ...
   /* DSReadPcap() handles pcap, pcapng, and .rtpdump format. Return value is packet length in bytes (or block data length for pcapng non-packet blocks). A return value of zero indicates end of file, -1 indicates an error condition (in which case an error message has already been displayed/logged) */
      ...
      if (pkt_len > 0) {
         ...
         int link_len = getLinkLayerlen(thread_info[tId].link_layer_info[nStream]);
      ...
      if (pkt_len < 0) return pkt_len;  /* error condition */
  ...
  */
      ...
      if (pkt_len - NOMINAL_MTU > 0) {  /* check for packet size larger than input cache data buffer nominal MTU size, for infrequent reasons as noted above. NOMINAL_MTU is defined in pktlib.h */
         ...
         if (!(uSuppressPacketInfoMessages & 1) && (thread_info[tId].num_oversize_nonfragmented_packets[nStream] < 3 || fUnexpectedCase)) {  /* display and log event message with some details */
...
int ret_val;  /* DSOpenPcap() return value */
int nStream = 0;  /* stream index */
char tmpstr[1024];
unsigned int uFlags;

   if (thread_info[thread_index].uErrorCondition) return;
...
/* open input pcap files, advance file pointer to first packet. Abort program on any input file failure */
   ...
   #pragma GCC diagnostic pop
      ...
      if (strcasestr(MediaParams[cmd_line_input].Media.inputFilename, ".pcap") || strcasestr(MediaParams[cmd_line_input].Media.inputFilename, ".rtp")) {  /* look for .pcap and .pcapng, JHB Oct 2020. Look for .rtp and .rtpdump, JHB Nov 2023 */
         ...
         if ((ret_val = DSOpenPcap(MediaParams[cmd_line_input].Media.inputFilename, uFlags, &thread_info[thread_index].pcap_in[nStream], thread_info[thread_index].pcap_file_hdr[nStream], "")) < 0) {  /* for error conditions, DSOpenPcap() shows messages */

            if (ret_val == -1) {  /* if file not found try up one subfolder */

               strcpy(tmpstr, "../");
               strcat(tmpstr, MediaParams[cmd_line_input].Media.inputFilename);

               if ((ret_val = DSOpenPcap(tmpstr, uFlags, &thread_info[thread_index].pcap_in[nStream], thread_info[thread_index].pcap_file_hdr[nStream], "")) < 0) {

                  fprintf(stderr, "Failed to open input file %s, input stream = %d, thread_index = %d, DSOpenPcap ret val = %d \n", tmpstr, nStream, thread_index, thread_info[thread_index].link_layer_info[nStream]);  /* still not found */
               }
            }
         }

         if (ret_val < 0) {  /* file not found, or was found but has error conditions */

            thread_info[thread_index].pcap_in[nStream] = NULL;
            thread_info[thread_index].uErrorCondition = 1;  /* error condition for at least one input file, error message is already printed and/or logged */
            break;  /* break out of while loop */
         }

         thread_info[thread_index].link_layer_info[nStream] = ret_val;  /* save link layer info */
         ...
         thread_info[thread_index].link_layer_info[nStream] = IO_TYPE_BER << 16;
...
valid_input_spec:
   ...
   if (cmd_line_input == 0) thread_info[thread_index].uErrorCondition = 1;  /* error if no inputs */

   if (thread_info[thread_index].uErrorCondition) app_printf(APP_PRINTF_NEW_LINE | APP_PRINTF_PRINT_ONLY, cur_time, thread_index, " *************** inside input setup, init err true, thread_index = %d", thread_index);
}
...
*/
   ...
   if (strlen(szOutputMediaPath) > 0 && szOutputMediaPath[strlen(szOutputMediaPath)-1] != '/') strcat(szOutputMediaPath, "/");  /* szOutputMediaPath is global declared in cmd_line_interface.c, add / suffix, if not already there */
...
unsigned int uFlags;
   ...
   if (group_idx >= 0 && !thread_info[thread_index].fp_pcap_group[group_idx]) {  /* group idx must be valid and a file handle available (initial stream group member is the owner), JHB Sep 2024 */
      ...
      int ret_val = DSOpenPcap(filestr, uFlags, &thread_info[thread_index].fp_pcap_group[group_idx], NULL, "");  /* open pcap for writing (uFlags contains DS_WRITE) */
      ...
      if (ret_val < 0) {
         ...
         thread_info[thread_index].uErrorCondition = 1;  /* error condition for at least one output file, error message is already printed and/or logged */
      }
...
unsigned int uFlags;
   ...
   if (thread_info[thread_index].uErrorCondition || !(Mode & ENABLE_JITTER_BUFFER_OUTPUT_PCAPS)) return;  /* do nothing if ENABLE_JITTER_BUFFER_OUTPUT_PCAPS is not set in -dN command line options */
...
unsigned int uFlags;
   ...
   if (DSConfigMediaService(NULL, uFlags, num_pktmed_threads, packet_flow_media_proc, NULL) < 0) {  /* start packet/media thread(s) */
      ...
      thread_info[MasterThread].uErrorCondition = 2;  /* non I/O related error condition */
      return -1;
   }
...
/* enable and configure event log */
   ...
   if (setup_type == LOG_EVENT_SETUP) {
      ...
      if (!fCreateDeleteTest && !fCapacityTest && (Mode & ENABLE_DEBUG_STATS)) dbg_cfg->uEventLogMode |= LOG_SET_API_STATUS;  /* for functional tests, enable API status and error numbers */
   ...
   /* DSInitLogging() in event_logging.cpp (diaglib) will create the log file, or if append mode is specified then open it for appending (see uEventLogMode enums in config.h) */
      ...
      if (strlen(szEventLogPath) > 0) {  /* szEventLogPath will be non-empty if --event_log_path is given on the cmd line, JHB Sep 2025 */
         ...
         strcat(dbg_cfg->szEventLogFilePath, szEventLogFile);
      }
      else strcpy(dbg_cfg->szEventLogFilePath, szEventLogFile);
...
/* more actions, including repeat mode and auto-quit */
   ...
   if (thread_info[thread_index].uErrorCondition) ret_val = 0;
...
uint8_t output_type;
   ...
   if ((output_type = (uFlags & 0x0f)) && p_pkt_len && *p_pkt_len > 0) {  /* output to pcap if specified by uFlags */

      static bool fOnce = false;
      static FILE* fp_pcap_out = NULL;
      char temp_filename[256] = "";

      if (isOutputBer(output_type)) strcpy(temp_filename, "ber_output.pcap");
      else if (isOutputAsn_Xml(output_type)) strcpy(temp_filename, "hi3_output.pcap");
