/*
 ...
 Documentation
  ...
  (as of Oct 2019, most recent doc is https://signalogic.com/documentation/SigSRF/SigSRF_Software_Documentation_R1-8.pdf)
 ...
 Revision History
   ...
   Modified Jan 2023 JHB, in PushPackets() use DS_PKT_INFO_PKTINFO flag in DSGetPacketInfo() to help minimize packet handling overhead
   ...
   Modified Nov 2023 JHB, implement PCAP_TYPE_RTP to handle .rtpdump (and .rtp) format inputs. Heavy lifting is done in pktlib DSOpenPcap() and DSReadPcapRecord(). See the modified pcap_hdr_t struct in pktlib.h
...
char tmpstr[MAX_APP_STR_LEN];  /* very large string used for various purposes */
            ...
            while (!fPMMasterThreadExit) {
               ...
               if (fPMThreadsClosing && !uQuitMessage) {
                  ...
                  if (use_log_file) {  /* use_log_file is set if packet history logging is enabled; see LoggingSetup() below. Cmd line entry -L[filename] enables packet history logging */
                     ...
                     sprintf(&tmpstr[strlen(tmpstr)], " and packet history logging and analyis of %d input packets and %d output packets", num_input_pkts, num_pulled_pkts);
                  }
...
/* print final mediaMin stats - session summary, stream group output timing */
   ...
   /* note stats are concatenated into one string and one call to app_printf() prior to display and logging; this avoids fragmentation and mixing with other messages when multiple app threads and packet/media threads are running */
      ...
      sprintf(&tmpstr[strlen(tmpstr)], "\n%s%sTCP redundant discards = ", tabstr, tabstr);
...
uint16_t hdr_type, dest_port = 0, src_port = 0, pyld_len = 0;
uint8_t protocol = 0;
...
read_packet:
         ...
         if (input_type == PCAP_TYPE_LIBPCAP || input_type == PCAP_TYPE_PCAPNG || input_type == PCAP_TYPE_RTP) {  /* handle pcap formats here, BER and other file formats below */
            ...
            if (pkt_len > 0) {  /* add non IP packet handing, JHB Dec2021 */
               ...
               if (hdr_type == ETH_P_ARP) {  /* ignore ARP packets (ETH_P_ARP defined in if_ether.h Linux header file, typically value of 0x0806) */
            ...
            /* fill in PktInfo struct with IP, UDP, and RTP header items */
               ...
               if (nCut > 0) { nCut--; goto read_packet; }  /* handle --cut if entered on cmd line, JHB Nov 2023 */
   ...
   if ((PktInfo.dst_port != SIP_UDP_PORT && PktInfo.src_port != SIP_UDP_PORT) && PktInfo.protocol == UDP_PROTOCOL) {  /* avoid SIP packets */
      ...
      for (unsigned int i=0; i<sizeof(filt_rtp_ssrc)/sizeof(uint32_t); i++) if (rtp_ssrc == filt_rtp_ssrc[i]) {  /* search list of ssrcs */

         uint32_t rtp_seqnum = DSGetPacketInfo(-1, DS_BUFFER_PKT_IP_PACKET | DS_PKT_INFO_RTP_SEQNUM, pkt_in_buf, -1, NULL, NULL);

         if (!fSessionChangeFound && rtp_seqnum == 50980) {

            fSessionChangeFound = true;
            #if 1
            printf("\n *** for stream 0x%x seq num %d changing payload type value %d \n", rtp_ssrc, rtp_seqnum, PktInfo.rtp_pyld_type);
            #else
            printf("\n *** for stream 0x%x seq num %d changing source port value %d \n", rtp_ssrc, rtp_seqnum, PktInfo.src_port);
            #endif
         }

         if (fSessionChangeFound) pkt_in_buf[PktInfo.ip_hdr_len+UDP_HEADER_LEN+1] = ++PktInfo.rtp_pyld_type;  /* change payload type for all remaining packets in the target stream */
      }
   }
#endif
   ...
   if ((PktInfo.dst_port != SIP_UDP_PORT && PktInfo.src_port != SIP_UDP_PORT) && PktInfo.protocol == UDP_PROTOCOL) {  /* avoid SIP packets */
      ...
      uint32_t rtp_ssrc = DSGetPacketInfo(-1, DS_BUFFER_PKT_IP_PACKET | DS_PKT_INFO_RTP_SSRC, pkt_in_buf, -1, NULL, NULL);
      uint32_t filt_rtp_ssrc[] = { 0x98e28577 };
      ...
      for (unsigned int i=0; i<sizeof(filt_rtp_ssrc)/sizeof(uint32_t); i++) if (rtp_ssrc == filt_rtp_ssrc[i]) {  /* search list of ssrcs */
         ...
         goto read_packet;
            ...
            if (pkt_len > 0 && (Mode & ENABLE_INTERMEDIATE_PCAP)) {  /* format BER data into TCP/IP packet and generate pcap output if specified in cmd line flags, JHB Dec 2021*/
               ...
               ret_val = DSGetPacketInfo(-1, DS_BUFFER_PKT_IP_PACKET | DS_PKT_INFO_PKTINFO, pkt_in_buf, -1, &PktInfo, NULL);  /* update PktInfo */
            }
         }
...
//  if (pkt_index) printf("updating pkt index = %d, prev index = %d, timestamp = %llu \n", pkt_index, thread_info[thread_index].EncapsulatedStreamIndex[j], (unsigned long long)pkt_timestamp);
               ...
               if (fFoundEncapsulatedCCPkt) {  /* CC UDP packet found, set protocol to UDP/RTP, update PktInfo */
                  ...
                  ret_val = DSGetPacketInfo(-1, DS_BUFFER_PKT_IP_PACKET | DS_PKT_INFO_PKTINFO, pkt_in_buf, -1, &PktInfo, NULL);  /* note we allow error messages here because we expect UDP/RTP and would like to know if that's not the case ... might need to change this, JHB Jan 2023*/
      ...
      /* mediaMin SDK version currently ignores TCP/IP packets other than encapsulated DER streams and SIP invites */
         ...
         /* look for SIP invite packets */
            ...
            goto read_packet;  /* continue reading from this input, JHB Dec 2020 */
         }
