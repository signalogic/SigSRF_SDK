/*
 ...
 Revision History
   ...
   Modified Jan 2023 JHB, filter non-dynamic UDP ports and allow list. See NON_DYNAMIC_UDP_PORT_RANGE, NonDynamic_UDP_Port_Allow_List[], and FILTER_UDP_PACKETS
   ...
   Modified Jan 2023 JHB, in PushPackets() use DS_PKT_INFO_PKTINFO_STRUCT flag in DSGetPacketInfo() to help minimize packet handling overhead
...
char tmpstr[MAX_APP_STR_LEN];
   ...
   if (isMasterThread) {  /* isMasterThread defined in mediaMin.h */
      ...
      else                                      printf("  no packet push rate method defined\n");
...
//         if (Mode & DISABLE_OUTPUT_BUF_INTERVAL) printf("  output buffer interval field in termN definitions is disabled\n");
      ...
      if (Mode & ENABLE_STREAM_SDP_INFO) printf("  in-stream SDP info enabled\n");
      if (Mode & DISABLE_TERMINATE_STREAM_ON_BYE) printf("  stream termination on SIP BYE message disabled\n");
      ...
      if (Mode & ENABLE_WAV_OUTPUT_TIMESTAMP_MATCH) {
         ...
         printf("  generate timestamp-matched wav output\n");
      }
   ...
   /* get a platform handle from DirectCore lib. Notes:
      ...
      -DirectCore detects and manages VM flags (e.g. rtdscp support), number of cores, concurrency, containers, etc
      -DirectCore manages shared mem files (/dev/shm/hwlib*) that are shared between processes
      ...
      -only one app thread should call DSAssignPlatform()
 */
...
/* all initialization complete, begin continuous packet push-pull loop */
   ...
   do {
      ...
      if (fPause) continue;  /* see if keyboard interactive pause is in effect */
...
/* print final mediaMin stats - session summary, stream group output timing */
   ...
   /* note stats are concatenated into one string and one call to app_printf() prior to display and logging; this avoids fragmentation and mixing with other messages when multiple app threads and packet/media threads are running */
      ...
      sprintf(&tmpstr[strlen(tmpstr)], "\n%s%sTCP redundant discards = ", tstr, tstr);
   ...
   /* if specified, display timestamp-match mode stats, JHB Sep 2023 */

      if (Mode & ENABLE_WAV_OUTPUT_TIMESTAMP_MATCH) {

         char szMergeFilename[2*CMDOPT_MAX_INPUT_LEN] = "md5sum ";  /* extra space for string manipulation */

         if (DSGetMergeWavFilenameTSM(&szMergeFilename[strlen(szMergeFilename)])) {  /* get merge wav filename from timestamp matching API in streamlib */

            char md5str[100], szFilename[2*CMDOPT_MAX_INPUT_LEN];
            int ret_val = -1;

            FILE *fp = popen(szMergeFilename, "r");  /* use popen() to execute md5sum cmd line and fscanf() to retrieve cmd line output */
            if (fp) { ret_val = fscanf(fp, "%s %s", md5str, szFilename); pclose(fp); }

            if (ret_val == 2) {  /* if it runs correctly, md5sum will output 2 values: (i) md5 hash result (ii) filename */
               sprintf(&tmpstr[strlen(tmpstr)], "%stimestamp-match mode", tstr);
               sprintf(&tmpstr[strlen(tmpstr)], "\n%s%smd5sum = %s %s", tstr, tstr, md5str, szFilename);
               strcat(tmpstr, "\n");
            }
         }
      }
   ...
   /* if specified, display packet arrival stats, JHB Aug 2023 */
      ...
      if ((Mode & USE_PACKET_ARRIVAL_TIMES) && (Mode & SHOW_PACKET_ARRIVAL_STATS)) {
         ...
         sprintf(&tmpstr[strlen(tmpstr)], "%sarrival timing [session]", tstr);

         sprintf(&tmpstr[strlen(tmpstr)], "\n%s%sdelta avg/max (msec) = ", tstr, tstr);
         ...
         sprintf(&tmpstr[strlen(tmpstr)], "\n%s%sjitter avg/max (msec) = ", tstr, tstr);
      ...
      if (thread_info[thread_index].fDynamicSessions) {
         ...
         for (i=0; i<thread_info[thread_index].dynamic_session_stats_index; i++) {
            ...
            char szSessInfo[200];
            sprintf(szSessInfo, "%s%s[%d] hSession %d, codec = %s, bitrate = %d, payload type = %d, ssrc = 0x%x \n", tstr, tstr, i, thread_info[thread_index].DynamicSessionStats[i].hSession, thread_info[thread_index].DynamicSessionStats[i].codecstr, thread_info[thread_index].DynamicSessionStats[i].bitrate, thread_info[thread_index].DynamicSessionStats[i].payload_type, thread_info[thread_index].DynamicSessionStats[i].ssrc);
...
/* clean up and exit */
   ...
   if (isMasterThread) {
      ...
      DSCloseLogging(0);  /* close event logging. DSCloseLogging() is in diaglib */
   }
...
/* copy RTP payload type to key (but not DTMF packets, which must match an existing session, JHB May 2019) */
   ...
   if (pyld_size != 4) {
      ...
      memcpy(&key[len], &rtp_pyld_type, sizeof(uint8_t));
      len += sizeof(uint8_t);
   }
   #endif
...
/* see if we already know about this stream */
   ...
   if ((!fInitKeys || thread_info[thread_index].fDynamicSessions) && !found_match) return 1;
   else return 0;
}
...
/* detect_codec_type_and_bitrate() uses an ad-hoc algorithm to make a best guess at codec type and bitrate. Notes:
...
/* added 0x21 check to pick up AMR-WB in 13041.0 pcap, JHB Feb2019 */
/* added 0x24 check to pick up AMR-WB for AMR-WB 12.65 7-byte SID, JHB Dec2020 */

   if ((codec_type == AMR || codec_type == AMR_WB) ||  /* add non-auto-detect: AMR-xx codec type already set (e.g. SDP file input), JHB Jan2021 */
...
// AMR-WB 12.65 kbps SID: 24e4e77e584c80
      ...
      switch (rtp_pyld_len) {
         ...
         case 33:
            if ((!codec_type || codec_type == AMR_WB) && !*bitrate) *bitrate = 12650;  /* notes:  1) this is a kludge fall-through for some AMR-WB SID cases, for example AMR-WB 23850 SID vs 12650 SID there will be a problem, 2) payload size 33 conflicts with EVS 13200 compact header format */
         case 37:
   ...
   switch (rtp_pyld_len) {
      ...
      case 61:
         if (codec_type == AMR_WB || (!codec_type && (rtp_pkt[0] & 0xf8) == 0xf0)) {
            return AMR_WB;  /*  AMR-WB 23850 bwe */
         }
      ...
      case 62:
         if (codec_type == AMR_WB || (!codec_type && !(rtp_pkt[0] & 0x0f) && rtp_pyld_len > 6)) {
            *bitrate = 23850;
            return AMR_WB;  /* AMR-WB 23850 octet-aligned with CMR byte (see test_files/AMR_WB.pcap for example) */
         }
         else if (!codec_type || codec_type == EVS) {
            *bitrate = 24400;
...
/* perform thorough packet validation before creating a session. Notes:
   ...
   -we could use DSGetPacketInfo() with the DS_PKT_INFO_PKTINFO_STRUCT flag, which would retrieve all of the items below in a PktInfo struct (see pktlib.h)
   -but we want fine-grained error messages so we use individual DS_PKT_INFO_xxx flags
   -PushPackets() uses the DS_PKT_INFO_PKTINFO_STRUCT flag for higher performance (minimize packet handling overhead)
*/
...
non_rtp_packet:
   ...
   rtp_seqnum = DSGetPacketInfo(-1, DS_BUFFER_PKT_IP_PACKET | DS_PKT_INFO_RTP_SEQNUM, pkt, pkt_len, NULL, NULL);
...
err_msg:
      if (fShowErrDebugInfo) {
         ...
         if (fPrevErr)
           fprintf(stderr, "%s%s, IP ver %d, payload type %d, ssrc = 0x%x, seq num = %d, pkt len %d, RTP pyld size %d, cat %d, pyld[0] %d, pyld[1] %d, pyld[2] %d \n", fPrevErr ? "" : "\n", errstr, ip_version, rtp_pyld_type, rtp_ssrc, rtp_seqnum, pkt_len_lib, rtp_pyld_len, cat, pkt[rtp_pyld_ofs], pkt[rtp_pyld_ofs+1], pkt[rtp_pyld_ofs+2]);
...
// if (!(Mode & COMBINE_INPUT_SPECS) && nInput > 0) printf("\n ============= group_id = %s, prev group id = %s \n", group_id, thread_info[thread_index].szGroupName[nInput-1]);
      ...
      if (!fStressTest && !fCapacityTest) {
         ...
         sprintf(session->szSessionName, "%s%s", szStreamGroupOutputPath, thread_info[thread_index].szGroupName[nInput]);  /* set session->szSessionName for wav outputs. Note we currently don't allow wav outputs during capacity test */
      }
...
//#define FORCE_EVS_16KHZ_OUTPUT  /* define this if EVS decoder output should be 16 kHz for stream groups, JHB Mar 2019 */
         ...
         /* set decode (output) Fs. For static sessions termN.sample_rate may be read from session config file */
            ...
            session->term1.attr.voice_attr.u.evs.codec_flags = DS_EVS_FS_16KHZ | (DS_EVS_BITRATE_13_2 << 2);  /* for EVS with ASR specified or without a stream group we set to 16 kHz and 13200 bps, no other flags set.  See "evs_codec_flags" in shared_include/session.h */
...
/* dynamic sessions are unidirectional (i.e. for bidirectional streams discovered in input packet flow 2 sessions are created), so here we set arbitrary IP addr and UDP port values for term2. Note - for static sessions term2 values are read from session config file or set via user app, creating bidirectional sessions */
   ...
   if (uTimestampMatchMode & TIMESTAMP_MATCH_MODE_ENABLE) {  /* for term2 transcoded audio we use codec type NONE if we are generating timestamp-matched wav output. packet/media threads treat codec type NONE as raw audio (16-bit). Note this is not a CLEARMODE codec, JHB Aug 2023 */
      session->term2.codec_type = DS_VOICE_CODEC_TYPE_NONE;
      session->term2.bitrate = 128000;
   }
...
/* group term setup */
   ...
   /* specify stream group output codec */
   ...
   /* specify stream group output sampling rate. Notes:
        ...
        -this is independent of the stream group output codec. If Fs rates are different, then down-sampling rate conversion is performed prior to codec encode (look for DSConvertFs() in DSProcessAudio() in audio_domain_pocessing.c)
        -ASR requires wideband audio for accuracy reasons
   */
      if (Mode & ENABLE_STREAM_GROUP_ASR) session->group_term.sample_rate = 16000;
      else session->group_term.sample_rate = 8000;
...
/* display dynamic session creation info on stdout and print to event log */
   ...
   sprintf(tmpstr2,
           "Creation packet info: IP ver %d, ssrc = 0x%x, seq num = %d, payload type %d, pkt len %d, RTP payload size %d, cat %d, rtp_pkt[0] 0x%x, src port = %u, dst_port = %u",
           ip_version, rtp_ssrc, rtp_seqnum, rtp_pyld_type, pkt_len_lib, rtp_pyld_len, cat, pkt[rtp_pyld_ofs], DSGetPacketInfo(-1, DS_BUFFER_PKT_IP_PACKET | DS_PKT_INFO_SRC_PORT, pkt, -1, NULL, NULL), DSGetPacketInfo(-1, DS_BUFFER_PKT_IP_PACKET | DS_PKT_INFO_DST_PORT, pkt, -1, NULL, NULL));
...
read_packet:
         ...
         if (input_type == PCAP_TYPE_LIBPCAP || input_type == PCAP_TYPE_PCAPNG) {  /* handle pcaps here, BER and other files below */
            ...
            if (pkt_len > 0) {  /* add non IP packet handing, JHB Dec2021 */
               ...
               if (pkt_type == ETH_P_ARP) {  /* ignore ARP packets (ETH_P_ARP defined in if_ether.h Linux header file, typically value of 0x0806) */
            ...
            /* fill in PktInfo struct with IP, UDP, and RTP header items */
               ...
               if (protocol != UDP_PROTOCOL && protocol != TCP_PROTOCOL) { /* ignore ICMP and various other protocols. Contact Signalogic if you need help implementing a new protocol or have protocol-related suggestions for the developers. Examples might include new encapsulation protocols, new media codecs, etc. */
   ...
   if (nCount < 2) {
      ...
      uint16_t src_port = DSGetPacketInfo(-1, DS_BUFFER_PKT_IP_PACKET | DS_PKT_INFO_SRC_PORT, pkt_in_buf, -1, NULL, NULL);
      if (src_port != 5060 && dst_port != 5060) {
         uint8_t rtp_pyld_type = DSGetPacketInfo(-1, DS_BUFFER_PKT_IP_PACKET | DS_PKT_INFO_RTP_PYLDTYPE, pkt_in_buf, -1, NULL, NULL);
         if (rtp_pyld_type == 105) { nCount++; if (nCount == 2) printf("\n\n two 105 packets omitted \n"); goto read_packet; }  /* don't push first two 105 packets */
      }
   }
#endif
   ...
   if (fFilter) {  /* if found act on filtered ssrc */
      ...
      goto read_packet;  /* filter this ssrc -- don't process its packet */
            ...
            if (pkt_len > 0 && (Mode & ENABLE_INTERMEDIATE_PCAP)) {  /* format BER data into TCP/IP packet and generate pcap output if specified in cmd line flags, JHB Dec 2021*/
               ...
               packet_actions(ber_data, pkt_in_buf, (protocol = TCP_PROTOCOL), &pkt_len, PCAP_TYPE_BER);
               ret_val = DSGetPacketInfo(-1, DS_BUFFER_PKT_IP_PACKET | DS_PKT_INFO_PKTINFO_STRUCT, pkt_in_buf, -1, &PktInfo, NULL);  /* update PktInfo */
            }
         }
      ...
      */
      ...
      /* DER encoded encapsulated stream processing, JHB Mar2021 */
         ...
         /* DSFindDerStream() finds info at HI2 level, including 1) new DER streams, 2) new destination ports for an existing DER stream */
            ...
            if (DSFindDerStream(pkt_in_buf, uFlags, &szInterceptPointId[0], der_dest_port_list, thread_info[thread_index].hFile_ASN_XML[j]) > 0) {
            ...
            /* DSDecodeDerStream() parses/decodes a DER stream looking for CC packets, and if found returns as fully formed IPv4/6 UDP packets */
               ...
               if ((cc_pktlen = DSDecodeDerStream(hDerStream, pkt_in_buf, pkt_out_buf, uFlags, &der_decode, thread_info[thread_index].hFile_ASN_XML[j])) > 0) {  /* 0 means nothing found, < 0 is an error condition, > 0 is length of found packet */
...
//  if (pkt_index) printf("updating pkt index = %d, prev index = %d, timestamp = %llu \n", pkt_index, thread_info[thread_index].EncapsulatedStreamIndex[j], (unsigned long long)pkt_timestamp);
               ...
               if (fFoundEncapsulatedCCPkt) {  /* CC UDP packet found, set protocol to UDP/RTP, update PktInfo */
                  ...
                  ret_val = DSGetPacketInfo(-1, DS_BUFFER_PKT_IP_PACKET | DS_PKT_INFO_PKTINFO_STRUCT, pkt_in_buf, -1, &PktInfo, NULL);  /* note we allow error messages here because we expect UDP/RTP and would like to know if that's not the case ... might need to change this, JHB Jan 2023*/

                  if (PktInfo.protocol != UDP_PROTOCOL) Log_RT(3, "mediaMin WARNING: DER decoded packet not UDP format \n");
                  else protocol = PktInfo.protocol;
               }
            ...
            if (msec_elapsedtime < msec_timestamp) {  /* push packet when elapsed time >= packet timestamp */
               ...
               if (isMasterThread) {
                  ...
                  if (((wait_time = msec_timestamp - msec_elapsedtime) > 2000 || last_wait_check_time) && msec_elapsedtime - last_wait_check_time > 1000) {
                     ...
                     if (protocol == UDP_PROTOCOL && (PktInfo.rtp_pyld_type >= RTCP_PYLD_TYPE_MIN && PktInfo.rtp_pyld_type <= RTCP_PYLD_TYPE_MAX)) fReseek = false;  /* don't allow RTCP packets to restart a pause, verify with 6537.0 test-case. To-do: there are pauses in other payload type ranges also, we can add later, JHB Jun 2023 */
                     else {
                        ...
                        if (++waiting_inputs >= thread_info[thread_index].nInPcapFiles) {  /* print waiting status only if all inputs are waiting */
                           if (wait_time/1000 > 0) app_printf(APP_PRINTF_SAME_LINE | APP_PRINTF_PRINT_ONLY, thread_index, "\rWaiting %llu of %llu sec pause in packet arrival times ...", (long long unsigned int)wait_time/1000, (long long unsigned int)wait_pause/1000);
            ...
            if ((nMsgTypeFound = ProcessSessionControl(pkt_in_buf, SESSION_CONTROL_SHOW_SIP_INVITE_MESSAGES | ((Mode & ENABLE_STREAM_SDP_INFO) ? SESSION_CONTROL_ADD_SIP_INVITE_SDP_INFO | SESSION_CONTROL_ADD_SAP_SDP_INFO : 0), j, thread_index, szKeyword)) > 0) {
               ...
               switch (nMsgTypeFound) {
                  ...
                  case SESSION_CONTROL_FOUND_SIP_BYE:
                     ...
                     if (!(Mode & DISABLE_TERMINATE_STREAM_ON_BYE)) {  /* default is to act on SIP BYE messages embedded in the stream */
               ...
               udp_ignore_count = 0;  /* reset the ignore state machine */
            }
            else {
...
ignore_udp_packet:
      ...
      */
         ...
         if (rtp_pyld_len <= 0 && fShowWarnings) {
            Log_RT(4, "mediaMin INFO: PushPackets() says unknown UDP packet (DSGetPacketInfo says invalid RTP payload size %d), dest port = %u, pkt len = %d \n", rtp_pyld_len, dest_port, pkt_len);
            goto read_packet;
         }
...
check_for_duplicated_headers:
            ...
            if (thread_info[thread_index].fDynamicSessions || ((Mode & CREATE_DELETE_TEST_PCAP) && debug_test_state == CREATE)) {

//  static int chk_cnt = 0;
//  printf("checking for new session %d \n", chk_cnt++);
  
               if (FindSession(pkt_in_buf, pkt_len, rtp_pyld_type, rtp_pyld_len, thread_index) > 0) {

                  ret_val = CreateDynamicSession(pkt_in_buf, pkt_len, hSessions, session_data, thread_index, j, n);
                  ...
                  else {  /* error or problem of some type, remove the key created by FindSession() */
                  ...
                  if (!(Mode & COMBINE_INPUT_SPECS) && !thread_info[thread_index].nSessions[j] && !thread_info[thread_index].fDuplicatedHeaders[j]) {
            ...
            for (i=0; i<nSessions; i++) {
               ...
               chnum = DSGetPacketInfo(hSessions[i], DS_BUFFER_PKT_IP_PACKET | DS_PKT_INFO_CHNUM_PARENT | DS_PKTLIB_SUPPRESS_ERROR_MSG, pkt_in_buf, pkt_len, NULL, NULL);  /* get the stream's parent chnum (ignore SSRC) */ 
...
push:
                  ...
                  if (!(Mode & DYNAMIC_SESSIONS) && (ret_val & DS_PUSHPACKETS_ENABLE_RFC7198_DEDUP)) goto read_packet;  /* duplicate packet, read next packet */
                  ...
                  */
                     ...
                     if ((Mode & USE_PACKET_ARRIVAL_TIMES) && (Mode & SHOW_PACKET_ARRIVAL_STATS)) {
                        ...
                        float delta_timestamp = 0;
                        int ptime = DSGetSessionInfo(hSessions[i], DS_SESSION_INFO_HANDLE | DS_SESSION_INFO_PTIME, 1, NULL);  /* get session ptime in msec */
                        ...
                        if (last_rtp_pyld_len[i] > 8 && (delta_timestamp = msec_timestamp_fp - last_msec_timestamp[i]) < MAX_STATS_GAP) {  /* omit DTMFs and SIDs, gaps larger than 500 msec (for 20 sec nominal ptime) */
                           ...
                           arrival_avg_jitter[i] = (arrival_avg_jitter[i]*num_arrival_stats_pkts[i] + abs(delta_timestamp - ptime))/(num_arrival_stats_pkts[i]+1);

                           num_arrival_stats_pkts[i]++;

                           #if 0
                           static int count[10] = { 0 };
                           if (count[i]++ < 50) printf("\n[%d]%d msec_timestamp_fp = %4.2f, delta_timestamp = %4.2f, jitter = %4.2f\n", i, num_arrival_stats_pkts[i], msec_timestamp_fp, delta_timestamp, abs(delta_timestamp - arrival_avg_delta[i]));
                           #endif

                           arrival_max_delta[i] = max(arrival_max_delta[i], delta_timestamp);
                           arrival_max_jitter[i] = max(arrival_max_jitter[i], abs(delta_timestamp - ptime));
                        }
                     }
   ...
   */
      ...
      if ((Mode & AUTO_ADJUST_PUSH_RATE) && nSessions && thread_info[thread_index].pcap_in[j]) {  /* if input file no longer open, avoid jumping back to "read_packet:" and operating on closed file */
...
*/
   ...
   if (strlen(szStreamGroupOutputPath) > 0 && szStreamGroupOutputPath[strlen(szStreamGroupOutputPath)-1] != '/') strcat(szStreamGroupOutputPath, "/");  /* add / suffix, if not already there */
}
...
unsigned int uFlags;
   ...
   while (i < thread_info[thread_index].nSessionsCreated) {
      ...
      if (i2 >= 0) {
         ...
         #if 0  /* not used, ASR output text is handled in pktlib and streamlib, JHB Jan2021 */
         if (Mode & ENABLE_STREAM_GROUP_ASR) {
...
/* when ASR is enabled, we currently have the packet/media thread pre-emption warning turned off. Notes, JHB Jan2021

  -this is until we implement an inferlib thread (including Kaldi libs) running on the 2nd core of a physical core pair, JHB Jan2021
...
/* misc protocols we can report instead of displaying "ignoring UDP port ..." message */
   ...
   if (strlen(portstr)) { app_printf(APP_PRINTF_NEW_LINE | APP_PRINTF_PRINT_ONLY, thread_index, "%s packet found, dst port = %u \n", portstr, port); return 1; }
