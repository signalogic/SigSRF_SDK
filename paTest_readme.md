...
Fortunately debug efforts were eventually successful, culminating in a short "row marker" memory test that could be inserted into the software at various points and would catch the error in action, adding a specific, precisely timed event to the log data.  This allowed the related stress conditions to be identified, the nature of the problem to be characterized, and finally, DDR3 chip timing parameter changes to be implemented and then verified as the root cause resolution to the problem.
