...
* The combination of colors is similar to "inferno" or "magma" colormaps in Matlab and R, which are known as perceptually uniform colormaps, for which researchers have found that the human brain perceives data steps as equal steps in the color space.  This is important for convolutional neural networks based on machine vision, for which we need input images that approximate common real world images
