...
If -dN command line entry contains the ENABLE_JITTER_BUFFER_OUTPUT_PCAPS flag, jitter buffer output streams -- including all re-ordering, RFC 8108 SSRC changes, DTX expansion, and packet loss / timestamp repairs -- will be written to pcap files. For example, in this command line:
...
will be generated by adding a "_jbN.pcap" suffix to the first input command line argument. Note that depending on the number of command line inputs, and streams per input, there could be many pcap files generated.
