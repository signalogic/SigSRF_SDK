...
In the first example, processing is 40 times faster, and in the second 28.5 times faster. In the latter example acceleration is less due to total number of streams, codec type and bitrates, and media content. Amount of acceleration is also dependent on host system CPU clock rate, number of cores, and storage configuration.  Additional performance information - and how to determine the maximum acceleration without sacrificing media quality - is given in [Bulk Pcap Performance Considerations](#user-content-bulkpcapperformanceconsiderations) below.
