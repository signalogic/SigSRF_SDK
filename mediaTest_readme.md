...
In the second example, the child streams contain early media (ring tones), which appear as "rectangular bursts" in the above waveform display. Clicking on the :arrow_forward: button should play stream group output audio, formed by accurately combining input stream contributors at their precise time relative to other contributors. Wav files for stream group output and individual contributors are also available.
