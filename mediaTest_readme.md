...
As a quick reference, basic procedures for analyzing and playing packet audio from within Wireshark is given here. These instructions are for pcaps containing one RTP stream (not multiple streams).

1. First, if you run mediaMin with static session configuation, make sure your session config file has the correct payload type set for either G711 uLaw or ALaw.
